  ...
  describe('createNotificationHandler', () => {
    it('should reject if unauthorized', async () => {
      ...
      const result = await createNotificationHandler({
        data: { userId: 'user-1', type: 'test', title: 'Test', channel: 'in_app' },
      });
      expect(result).toEqual({ error: 'Unauthorized' });
    });
    it('should reject if non-admin', async () => {
      vi.mocked(auth.getSessionFromHeaders).mockResolvedValue(userSession);
      const result = await createNotificationHandler({
        data: { userId: 'user-1', type: 'test', title: 'Test', channel: 'in_app' },
      });
      expect(result).toEqual({ error: 'Unauthorized' });
  ...
  describe('listNotificationsHandler', () => {
    ...
    it('should return paginated notifications', async () => {
      ...
      mockDb.then
        ...
        .mockImplementationOnce((fn) => Promise.resolve(items).then(fn));
      const result = await listNotificationsHandler({ data: { page: 1, limit: 20 } });
