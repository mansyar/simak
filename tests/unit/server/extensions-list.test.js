...
describe('listExtensionRequestsHandler', () => {
  ...
  it('should return paginated results without status filter', async () => {
    vi.mocked(auth.getSessionFromHeaders).mockResolvedValue(instructorSession);
    mockDb.then
      .mockImplementationOnce((onfulfilled) => Promise.resolve([{ id: 1 }]).then(onfulfilled))
      .mockImplementationOnce((onfulfilled) => Promise.resolve([{ count: 2 }]).then(onfulfilled));
    ...
    mockDb.then = vi.fn(function (onfulfilled) {
...
describe('listMyExtensionRequestsHandler', () => {
  ...
  it('should return items with checkpoint info', async () => {
    ...
    mockDb.then
      .mockImplementationOnce((onfulfilled) => Promise.resolve([{ id: 1 }]).then(onfulfilled))
      .mockImplementationOnce((onfulfilled) => Promise.resolve(expectedItems).then(onfulfilled));
    const result = await listMyExtensionRequestsHandler({
      data: { assignmentId: 1 },
    });
