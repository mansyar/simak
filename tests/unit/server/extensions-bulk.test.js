...
describe('bulkExtendHandler', () => {
  ...
  it('should extend all unfinished checkpoints (locked, unlocked, submitted, under_review, revise)', async () => {
    ...
    mockDb.then
      .mockImplementationOnce((onfulfilled) => Promise.resolve([{ id: 1 }]).then(onfulfilled))
      .mockImplementationOnce((onfulfilled) => Promise.resolve(checkpoints).then(onfulfilled));
    const result = await bulkExtendHandler({ data: validInput });
  ...
  it('should notify the student after bulk extension', async () => {
    ...
    const notificationValues = valuesCalls.find((call) => call[0]?.type === 'deadline_extended');
