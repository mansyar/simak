...
describe('getInstructorDashboardDataHandler', () => {
  ...
  it('should return empty dashboard when instructor has no assignments', async () => {
    ...
    mockDb.then
      .mockImplementationOnce((fn) => Promise.resolve([]).then(fn))
      .mockImplementationOnce((fn) => Promise.resolve([]).then(fn))
      .mockImplementationOnce((fn) => Promise.resolve([]).then(fn));
    const result = await getInstructorDashboardDataHandler();
  ...
  it('should return pending review items with assignment data', async () => {
    ...
    mockDb.then
      // Query 1: instructorAssignments
      .mockImplementationOnce((fn) => Promise.resolve([{ id: 1 }]).then(fn))
      // Query 2: pendingReviewCount
      .mockImplementationOnce((fn) => Promise.resolve([{ count: 2 }]).then(fn))
      ...
      .mockImplementationOnce((fn) => Promise.resolve([{ assignmentId: 1, count: 3 }]).then(fn))
      // Query 7: pendingCounts
      .mockImplementationOnce((fn) => Promise.resolve([{ assignmentId: 1, count: 2 }]).then(fn))
  ...
  it('should deduplicate pending review items by checkpointId', async () => {
    ...
    mockDb.then
      .mockImplementationOnce((fn) => Promise.resolve([{ id: 1 }]).then(fn))
      .mockImplementationOnce((fn) => Promise.resolve([{ count: 3 }]).then(fn))
      ...
      .mockImplementationOnce((fn) => Promise.resolve([{ assignmentId: 1, count: 2 }]).then(fn))
      .mockImplementationOnce((fn) => Promise.resolve([{ assignmentId: 1, count: 1 }]).then(fn))
      .mockImplementationOnce((fn) =>
  ...
  it('should handle null submittedAt in pending review items', async () => {
    ...
    mockDb.then
      .mockImplementationOnce((fn) => Promise.resolve([{ id: 1 }]).then(fn))
      .mockImplementationOnce((fn) => Promise.resolve([{ count: 1 }]).then(fn))
      .mockImplementationOnce((fn) =>
        Promise.resolve([
          {
            submissionId: 100,
  ...
  it('should map checkpoint states to status labels', async () => {
    ...
    mockDb.then
      ...
      .mockImplementationOnce((fn) => Promise.resolve([{ count: 0 }]).then(fn))
