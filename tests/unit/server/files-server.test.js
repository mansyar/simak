...
vi.mock('@/lib/storage', () => ({
  generateFileKey: vi
    .fn()
    .mockImplementation((ext, prefix = 'submissions') => `${prefix}/mock-uuid.${ext}`),
...
function mockDbQuery(rows) {
  const limitStep = {
    then: (fn) => Promise.resolve(rows).then(fn),
  };
  const whereStep = {
...
describe('files.server.ts - getPresignedDownloadUrlHandler', () => {
  ...
  it('should reject when submission not found', async () => {
    ...
    vi.mocked(getDb).mockReturnValue(submissionDb);
    const { getPresignedDownloadUrlHandler } = await import('@/server/files.server');
