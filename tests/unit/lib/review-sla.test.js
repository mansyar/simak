...
describe('adjustDeadlinesForBreach', () => {
  ...
  it('should extend subsequent checkpoint dueDates', async () => {
    mockTx.then.mockImplementation((onfulfilled) =>
      Promise.resolve([
...
describe('dispatchSLABreachNotifications', () => {
  ...
  it('should query admin users with role filter', async () => {
    mockDb.then.mockImplementation((onfulfilled) => Promise.resolve(adminUsers).then(onfulfilled));
  ...
  it('should create in_app and email notifications per admin', async () => {
    ...
    const valuesCalls = mockDb.values.mock.calls.map((c) => c[0]);
    // Each admin should have in_app and email notifications
    const admin1Notifications = valuesCalls.filter((v) => v.userId === 'admin-1');
    const admin2Notifications = valuesCalls.filter((v) => v.userId === 'admin-2');
  ...
  it('should catch and log error when admin query fails', async () => {
    ...
    mockDb.then.mockImplementation((_onfulfilled, onrejected) => {
      onrejected(dbError);
    });
    const consoleSpy = vi.spyOn(console, 'error').mockImplementation(() => {});
    await expect(
