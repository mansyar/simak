...
vi.mock('lucide-react', () => ({
  Calendar: () => _jsx('div', { 'data-testid': 'calendar-icon' }),
  Clipboard: () => _jsx('div', { 'data-testid': 'clipboard-icon' }),
}));
...
describe('StudentAssignmentCard', () => {
  ...
  it('should render template type using Badge component', () => {
    render(_jsx(StudentAssignmentCard, { assignment: mockAssignment }));
  ...
  it('should not contain violet-500 gradient classes', () => {
    const { container } = render(_jsx(StudentAssignmentCard, { assignment: mockAssignment }));
