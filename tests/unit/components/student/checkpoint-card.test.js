...
vi.mock('@tanstack/react-router', () => ({
  Link: ({ children, className, to }) =>
    _jsx('a', { href: to, className: className, children: children }),
  useNavigate: () => vi.fn(),
}));
...
describe('CheckpointCard', () => {
  ...
  it('should render checkpoint name', () => {
    render(_jsx(CheckpointCard, { checkpoint: passedCheckpoint, assignmentId: 101 }));
  ...
  it('should render locked state badge with correct variant', () => {
    render(_jsx(CheckpointCard, { checkpoint: lockedCheckpoint, assignmentId: 101 }));
  ...
  it('should indicate overdue checkpoints', () => {
    render(_jsx(CheckpointCard, { checkpoint: overdueCheckpoint, assignmentId: 101 }));
  ...
  it('should render resubmit button for revise state', () => {
    const reviseCheckpoint = { ...passedCheckpoint, state: 'revise' };
    render(_jsx(CheckpointCard, { checkpoint: reviseCheckpoint, assignmentId: 101 }));
    expect(screen.getByText('studentAssignments.resubmit')).toBeDefined();
  });
  it('should render view submission link for submitted state', () => {
    const submittedCheckpoint = { ...passedCheckpoint, state: 'submitted' };
    render(_jsx(CheckpointCard, { checkpoint: submittedCheckpoint, assignmentId: 101 }));
  ...
  it('should render view submission link for under_review state', () => {
    const underReviewCheckpoint = { ...passedCheckpoint, state: 'under_review' };
    render(_jsx(CheckpointCard, { checkpoint: underReviewCheckpoint, assignmentId: 101 }));
    expect(screen.getByText('studentAssignments.viewSubmission')).toBeDefined();
  });
  ...
  it('should use semantic info token for submitted state container', () => {
    const submitted = { ...passedCheckpoint, state: 'submitted' };
    render(_jsx(CheckpointCard, { checkpoint: submitted, assignmentId: 101 }));
    const card = screen.getByText('Proposal').closest('[class*="border-l-"]');
  ...
  it('should use semantic warning token for under_review state container', () => {
    const underReview = { ...passedCheckpoint, state: 'under_review' };
    render(_jsx(CheckpointCard, { checkpoint: underReview, assignmentId: 101 }));
    const card = screen.getByText('Proposal').closest('[class*="border-l-"]');
  ...
  it('should use semantic error token for revise state container', () => {
    const revise = { ...passedCheckpoint, state: 'revise' };
    render(_jsx(CheckpointCard, { checkpoint: revise, assignmentId: 101 }));
    const card = screen.getByText('Proposal').closest('[class*="border-l-"]');
