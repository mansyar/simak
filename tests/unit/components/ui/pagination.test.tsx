...
describe('Pagination', () => {
  ...
  it('should render current/total label when showCounter is true', () => {
    render(
      <Pagination currentPage={2} totalPages={5} onPageChange={vi.fn()} showCounter />,
    );
  ...
  it('should render page numbers when showPageNumbers is true (up to 5)', () => {
    render(
      <Pagination currentPage={3} totalPages={10} onPageChange={vi.fn()} showPageNumbers />,
    );
    // Should show 5 page number buttons: 1, 2, 3, 4, 5
    const pageButtons = screen.getAllByRole('button').filter(
      (btn) => !btn.getAttribute('aria-label')?.match(/(previous|next) page/i),
    );
  ...
  it('should clamp page numbers at the start', () => {
    render(
      <Pagination currentPage={1} totalPages={10} onPageChange={vi.fn()} showPageNumbers />,
    );
    const pageButtons = screen.getAllByRole('button').filter(
      (btn) => !btn.getAttribute('aria-label')?.match(/(previous|next) page/i),
    );
  ...
  it('should clamp page numbers at the end', () => {
    render(
      <Pagination currentPage={10} totalPages={10} onPageChange={vi.fn()} showPageNumbers />,
    );
    const pageButtons = screen.getAllByRole('button').filter(
      (btn) => !btn.getAttribute('aria-label')?.match(/(previous|next) page/i),
    );
  ...
  it('should fire onPageChange when a page number is clicked', () => {
    ...
    const page5 = screen.getAllByRole('button').find(
  ...
  it('should highlight the current page number', () => {
    ...
    const page3 = screen.getAllByRole('button').find(
