...
describe('TemplateTypeBadge', () => {
  it('renders the type string', () => {
    render(_jsx(TemplateTypeBadge, { type: 'Research' }));
    expect(screen.getByText('Research')).toBeInTheDocument();
  });
  it('applies canonical pill class string', () => {
    render(_jsx(TemplateTypeBadge, { type: 'Assignment' }));
  ...
  it('renders with uppercase text via CSS', () => {
    render(_jsx(TemplateTypeBadge, { type: 'project' }));
    const badge = screen.getByText('project');
    expect(badge).toHaveClass('uppercase');
  });
  it('applies custom className', () => {
    render(_jsx(TemplateTypeBadge, { type: 'Type', className: 'custom-class' }));
    expect(screen.getByText('Type')).toHaveClass('custom-class');
  });
  it('renders as a span element', () => {
    render(_jsx(TemplateTypeBadge, { type: 'Lab' }));
