...
describe('EmptyState', () => {
  ...
  it('renders children when provided', () => {
    render(
      _jsx(EmptyState, { ...defaultProps, children: _jsx('button', { children: 'Create New' }) }),
    );
  ...
  it('does not render empty p when description is omitted', () => {
    const { container } = render(_jsx(EmptyState, { icon: FileQuestion, title: 'No items' }));
    const paragraphs = container.querySelectorAll('p');
    expect(paragraphs.length).toBe(0);
  });
  it('applies custom className', () => {
    const { container } = render(_jsx(EmptyState, { ...defaultProps, className: 'custom-class' }));
    const root = container.firstChild;
  ...
  describe('compact prop', () => {
    it('renders with default padding when compact is false', () => {
      const { container } = render(_jsx(EmptyState, { ...defaultProps, compact: false }));
      const root = container.firstChild;
    ...
    it('renders with reduced padding when compact is true', () => {
      const { container } = render(_jsx(EmptyState, { ...defaultProps, compact: true }));
      const root = container.firstChild;
