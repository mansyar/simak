...
describe('StatusDot', () => {
  it('renders verified variant with bg-success', () => {
    const { container } = render(<StatusDot variant="verified" />);
  ...
  it('renders label text when provided', () => {
    render(<StatusDot variant="verified" label="Verified" />);
    expect(screen.getByText('Verified')).toBeDefined();
  });

  it('renders label with correct text size', () => {
    render(<StatusDot variant="inactive" label="Inactive" />);
  ...
  it('applies custom className', () => {
    const { container } = render(<StatusDot variant="verified" className="custom-class" />);
    const root = container.firstChild as HTMLElement;
    expect(root.className).toContain('custom-class');
  });

  it('renders with gap between dot and label', () => {
    const { container } = render(<StatusDot variant="verified" label="Active" />);
    const root = container.firstChild as HTMLElement;
