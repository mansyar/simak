...
vi.mock('@tanstack/react-query', () => ({
  ...
  useMutation: (config: {
    ...
    mutate: (...args: unknown[]) => {
      const mutationFn = config?.mutationFn as ((...a: unknown[]) => Promise<unknown>) | undefined;
      if (mutationFn) {
...
vi.mock('qrcode.react', () => ({
  QRCodeSVG: ({ value }: { value: string }) => <div data-testid="qr-code">QR:{value}</div>,
}));
...
describe('TwoFactorSettings', () => {
  ...
  it('should filter TOTP input to digits only and max 6 chars', async () => {
    ...
    const totpInput = screen.getByLabelText('Authentication Code') as HTMLInputElement;
