...
describe('DueDatePreview', () => {
  ...
  it('should calculate cumulative due dates correctly', () => {
    ...
    const cp1Input = screen.getByTestId('due-date-input-1') as HTMLInputElement;
    expect(cp1Input.value).toBe('2026-06-15');

    // CP2: June 1 + 14 + 30 = July 15
    const cp2Input = screen.getByTestId('due-date-input-2') as HTMLInputElement;
    expect(cp2Input.value).toBe('2026-07-15');

    // CP3: June 1 + 14 + 30 + 7 = July 22
    const cp3Input = screen.getByTestId('due-date-input-3') as HTMLInputElement;
  ...
  it('should default to current date when no baseDate is provided', () => {
    ...
    render(<DueDatePreview checkpoints={defaultCheckpoints} overrides={[]} onOverride={vi.fn()} />);
  ...
  it('should render empty state when no checkpoints are provided', () => {
    render(
      <DueDatePreview checkpoints={[]} overrides={[]} onOverride={vi.fn()} baseDate={baseDate} />,
    );
