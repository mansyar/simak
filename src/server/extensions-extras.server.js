 ...
 */
async function calculateExtensionAdjustment(db, args) {
 ...
 */
export async function approveExtensionHandler(args) {
  ...
  try {
    ...
    await db.transaction(async (tx) => {
      ...
      if (request.checkpointId) {
        await calculateExtensionAdjustment(tx, {
 ...
 */
export async function rejectExtensionHandler(args) {
  const session = await getSessionFromHeaders();
  if (!isInstructor(session)) {
    return { error: 'Unauthorized' };
  }
 ...
 * Instructor-only, ownership-guarded.
 */
export async function bulkExtendHandler(args) {
  const session = await getSessionFromHeaders();
  if (!isInstructor(session)) {
    return { error: 'Unauthorized' };
  }
