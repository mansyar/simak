...
export const updateProfile = createServerFn({ method: 'POST' }).handler(async (args) => {
  const { updateProfileHandler } = await import('./settings.server');
  return updateProfileHandler(args);
});
export const updateUserSettings = createServerFn({ method: 'POST' }).handler(async (args) => {
...
export const getCurrentUser = createServerFn({ method: 'GET' }).handler(async (_args) => {
