...
const REVIEWABLE_STATES = ['submitted', 'under_review'];
/**
 ...
 */
export async function listPendingReviewsHandler(args) {
 ...
 */
export async function getReviewDetailHandler(args) {
  const session = await getSessionFromHeaders();
  if (!isInstructor(session)) {
    return { error: 'Unauthorized' };
  }
 ...
 */
export async function submitReviewHandler(args) {
  const session = await getSessionFromHeaders();
  if (!isInstructor(session)) {
    return { error: 'Unauthorized' };
  }
  ...
  if (!REVIEWABLE_STATES.includes(submission.checkpointState)) {
