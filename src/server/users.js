...
import { z } from 'zod';
const VALID_CREATE_ROLES = ['admin', 'instructor', 'student'];
const VALID_ROLES = ['superadmin', 'admin', 'instructor', 'student'];
...
export const updateUser = createServerFn({ method: 'POST' }).handler(async (args) => {
...
export const deleteUser = createServerFn({ method: 'POST' }).handler(async (args) => {
  const { deleteUserHandler } = await import('./users.server');
  const data = UserIdParamSchema.parse(args.data);
  return deleteUserHandler({ data });
});
export const generateSetupLink = createServerFn({ method: 'POST' }).handler(async (args) => {
