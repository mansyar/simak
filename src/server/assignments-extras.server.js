 ...
 */
export async function unlockCheckpointHandler(args) {
 ...
 */
export async function extendDeadlineHandler(args) {
  const session = await getSessionFromHeaders();
  if (!isInstructor(session)) {
    return { error: 'Unauthorized' };
  }
...
export async function listStudentAssignmentsHandler(args) {
  const session = await getSessionFromHeaders();
...
export async function getStudentAssignmentDetailHandler(args) {
  const session = await getSessionFromHeaders();
  if (!isStudent(session)) {
    return null;
  }
  ...
  const checkpointsWithConsults = await db
    .select({
      ...
      verifiedConsultationCount: sql`COALESCE(COUNT(CASE WHEN ${consultations.status} = 'verified' THEN 1 END), 0)::int`,
    })
    .from(checkpoints)
    .leftJoin(
      consultations,
