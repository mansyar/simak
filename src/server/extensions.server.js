 ...
 */
async function findActiveCheckpoint(db, assignmentId, studentId) {
 ...
 */
async function countActiveExtensionRequests(db, assignmentId, studentId) {
 ...
 */
export async function requestExtensionHandler(args) {
  ...
  try {
    ...
    await db.insert(notifications).values({
      ...
      message: `A student has requested a ${extensionDays}-day deadline extension for checkpoint.`,
      channel: 'in_app',
      metadata: {
 ...
 */
export async function listExtensionRequestsHandler(args) {
  const session = await getSessionFromHeaders();
 ...
 */
export async function listMyExtensionRequestsHandler(args) {
  const session = await getSessionFromHeaders();
  if (!isStudent(session)) {
    return { error: 'Unauthorized' };
  }
