...
export async function getUserHandler(args) {
  const session = await getSessionFromHeaders();
...
export async function createUserHandler(args) {
  const session = await getSessionFromHeaders();
  if (!session) {
...
export async function updateUserHandler(args) {
  const session = await getSessionFromHeaders();
  if (!session || (session.user.role !== 'admin' && session.user.role !== 'superadmin')) {
    return { error: 'Unauthorized' };
  }
...
export async function deleteUserHandler(args) {
  const session = await getSessionFromHeaders();
  if (!session || (session.user.role !== 'admin' && session.user.role !== 'superadmin')) {
    return { error: 'Unauthorized' };
  }
...
export async function generateSetupLinkHandler(args) {
  const session = await getSessionFromHeaders();
  if (!session || (session.user.role !== 'admin' && session.user.role !== 'superadmin')) {
    return { error: 'Unauthorized' };
  }
