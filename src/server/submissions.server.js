...
export async function submitCheckpointHandler(args) {
  ...
  const [versionResult] = await db
    .select({ maxVersion: sql`COALESCE(MAX(${submissions.version}), 0)::int` })
    .from(submissions)
...
export async function listSubmissionsHandler(args) {
  const session = await getSessionFromHeaders();
  if (!isStudent(session)) {
    return { error: 'Unauthorized' };
  }
...
export async function getSubmissionDetailHandler(args) {
  const session = await getSessionFromHeaders();
  if (!isStudent(session)) {
    return { error: 'Unauthorized' };
  }
