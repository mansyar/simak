  ...
  try {
    ...
    await Promise.allSettled([
      ...
      enqueueEventEmail({
        ...
        buildBody: () =>
          `<p>Student <strong>${opts.studentName}</strong> in <strong>${opts.assignmentTitle}</strong> is at <strong>${assessment.level}</strong> risk.</p>` +
          `<p>Risk factors: ${riskParams.riskFactors}</p>`,
      }),
    ]);
  } catch (err) {
