import { jsx as _jsx, jsxs as _jsxs } from 'react/jsx-runtime';
/// <reference types="vite/client" />
import { useState, createContext, useContext, useCallback } from 'react';
import { Outlet, createRootRoute, HeadContent, Scripts } from '@tanstack/react-router';
import { QueryClient, QueryClientProvider } from '@tanstack/react-query';
import globalCss from '../app/global.css?url';
import { detectLocale } from '../i18n/index';
import enTranslations from '../../locales/en.json';
import idTranslations from '../../locales/id.json';
const queryClient = new QueryClient();
const translations = {
  en: enTranslations,
  id: idTranslations,
};
function resolveKey(obj, key) {
  const parts = key.split('.');
  let current = obj;
  for (const part of parts) {
    if (typeof current === 'object' && current !== null && part in current) {
      current = current[part];
    } else {
      return key;
    }
  }
  return typeof current === 'string' ? current : key;
}
function interpolate(text, params) {
  if (!params) return text;
  return text.replace(/\{(\w+)\}/g, (_, p) => params[p] ?? `{${p}}`);
}
// Placeholder ThemeProvider — replaces the hook-based approach for SSR compatibility
function ThemeScript() {
  return _jsx('script', {
    dangerouslySetInnerHTML: {
      __html: `
          (function() {
            try {
              var theme = localStorage.getItem('simak-theme');
              if (!theme) {
                theme = window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light';
              }
              if (theme === 'dark') {
                document.documentElement.classList.add('dark');
              }
            } catch(e) {}
          })();
        `,
    },
  });
}
const I18nContext = createContext({
  locale: 'en',
  setLocale: () => {},
  t: (_key) => _key,
});
export function useI18n() {
  return useContext(I18nContext);
}
function useI18nProvider() {
  const [locale, setLocaleState] = useState(() => {
    if (typeof window === 'undefined') return 'en';
    return detectLocale();
  });
  const setLocale = useCallback((newLocale) => {
    setLocaleState(newLocale);
    if (typeof window !== 'undefined') {
      localStorage.setItem('simak-locale', newLocale);
    }
  }, []);
  const t = useCallback(
    (key, params) => {
      const raw = resolveKey(translations[locale], key);
      return interpolate(raw, params);
    },
    [locale],
  );
  return { locale, setLocale, t };
}
function NotFoundComponent() {
  const { t } = useI18n();
  return _jsxs('div', {
    className: 'flex min-h-screen flex-col items-center justify-center gap-4 p-4',
    children: [
      _jsx('h1', { className: 'text-4xl font-bold text-foreground', children: '404' }),
      _jsx('p', { className: 'text-lg text-muted-foreground', children: t('error.notFound') }),
      _jsx('a', {
        href: '/dashboard',
        className: 'text-primary hover:underline',
        children: t('common.goToDashboard'),
      }),
    ],
  });
}
export const Route = createRootRoute({
  notFoundComponent: NotFoundComponent,
  head: () => ({
    meta: [
      {
        charSet: 'utf-8',
      },
      {
        name: 'viewport',
        content: 'width=device-width, initial-scale=1',
      },
      {
        title: 'SIMAK — Sistem Informasi dan Manajemen Akademik',
      },
    ],
    links: [
      {
        rel: 'stylesheet',
        href: globalCss,
      },
    ],
  }),
  component: RootComponent,
});
function RootComponent() {
  return _jsx(RootDocument, { children: _jsx(Outlet, {}) });
}
function RootDocument({ children }) {
  const i18n = useI18nProvider();
  return _jsxs('html', {
    lang: i18n.locale,
    children: [
      _jsxs('head', { children: [_jsx(ThemeScript, {}), _jsx(HeadContent, {})] }),
      _jsxs('body', {
        children: [
          _jsx('div', {
            id: 'skip-to-content',
            children: _jsx('a', {
              href: '#main-content',
              className:
                'sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 focus:z-50 focus:px-4 focus:py-2 focus:bg-background focus:text-foreground focus:rounded',
              children: i18n.t('common.skipToContent'),
            }),
          }),
          _jsx(QueryClientProvider, {
            client: queryClient,
            children: _jsx(I18nContext.Provider, { value: i18n, children: children }),
          }),
          _jsx(Scripts, {}),
        ],
      }),
    ],
  });
}
