...
### Track: Gradebook & Final Grade Computation (TRACK-025) (July 2026)

- **Grade computation engine** — Pure function `computeFinalGrade(checkpoints, config)` in `src/lib/grade-computation.ts`; per-checkpoint scoring (pass/fail = 100/0, rubric = weighted avg of `review_scores` by criterion weight); grading schemes (equal_weight = simple avg, custom_weight = weighted by `templateCheckpointId` with sum=100 validation); letter grade bounds (default `{A:90,B:80,C:70,D:60}`, sorted descending, F fallback); status derivation (complete/in_progress/incomplete); stale weights detection with equal_weight fallback + warning flag
- **Schema & migration** — 2 new tables: `assignment_grade_config` (per-assignment config: gradingScheme pgEnum, customWeights jsonb, letterGradeBounds jsonb with defaults) and `final_grades` (per-student computed grade: numericScore numeric(5,2), letterGrade text, status pgEnum, contributingCheckpoints jsonb snapshot); unique constraints on (assignmentId) and (assignmentId, studentId); migration 0014 with backfill for pre-existing assignments
- **Server functions** — 4 gradebook handlers in `gradebook.ts` + `gradebook.server.ts`: `getStudentFinalGradeHandler` (ownership-verified, fetches config + checkpoints + review_scores), `getAssignmentGradebookHandler` (instructor/admin ownership, batch student grades, sorted by name), `saveGradeConfigHandler` (admin-only, upserts config, audit logs), `recomputeAllGradesHandler` (admin-only, batch recompute all students); Zod schemas with `superRefine` validation for custom weight sum=100
- **Grade recomputation triggers** — `recomputeStudentGrade(db, assignmentId, studentId)` in `reviews-extras.server.ts` called as advisory post-commit in `submitReviewHandler` (only when decision='pass'); `createDefaultGradeConfig(tx, assignmentId)` in `assignments-extras.server.ts` called inside `createAssignmentHandler` transaction (equal_weight, default letter bounds)
- **Instructor gradebook view** (`/instructor/assignments/$id/gradebook`) — Students × checkpoints matrix table with per-checkpoint scores (pass/fail badges for non-rubric, numeric for rubric), final grade column (score + letter badge + status badge), read-only config summary card, CSV + Excel export buttons, admin-only "Recompute All Grades" button with confirmation dialog, admin-only "Grade Settings" dialog button
- **Admin grade settings dialog** — `GradeSettingsDialog.tsx` with React Hook Form + Zod resolver; grading scheme Select (equal_weight/custom_weight), letter bound inputs (A/B/C/D), conditional custom weight inputs per checkpoint; validation via `SaveGradeConfigSchema` superRefine
- **Student final grade card** — `StudentFinalGradeCard.tsx` on `/student/assignments/$id`; fetches `getStudentFinalGrade` server function; displays numeric score + letter badge (complete only) + status badge; collapsible breakdown table with per-checkpoint scores (pass/fail labels or numeric scores); loading skeleton, null (no config), and error states
- **Admin grade distribution analytics** — 9th query in `getAdminAnalyticsDataHandler` counting `final_grades` by `letterGrade`; "Grade Distribution" card with Progress bars for A/B/C/D/F on `/admin/analytics`; conditional render when total grades > 0; not filtered by date range (across all assignments)
- **CSV/Excel export** — `exportGradebookCsvHandler` in `analytics-export.server.ts` (admin-only, dynamic checkpoint columns, formula injection mitigation via `escapeCsvValue`); `exportGradebookToExcel` in `excel-export.ts` (client-side .xlsx with sanitized cells via `sanitizeCell`)
- **Role-based access** — Instructor layout (`instructor.tsx`) extended to allow admin/superadmin for gradebook access; `RecomputeGradesButton` and `GradeSettingsDialog` only render when `isAdmin=true` in gradebook data
- **i18n** — 27 new keys in both EN and ID locales under `gradebook.*` namespace (title, studentName, finalGrade, numericScore, status.*, passed/notPassed, empty/noGrades, configSummary, configScheme, equalWeight/customWeight, letterBounds, customWeights, staleWeightsWarning, recomputeAll/recomputeConfirm/recomputeSuccess/recomputeError, exportCsv/exportExcel, loading/loadError, settings.*, student.*, analytics.gradeDistribution)
- **Tests** — 3,537 tests pass across 344 test files; coverage ≥80% on all thresholds (stmts 87.95%, branches 81.89%, funcs 83.54%, lines 88.56%)
