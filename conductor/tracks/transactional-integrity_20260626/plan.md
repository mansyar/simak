...
## Phase 6: Documentation & fileKey Investigation [checkpoint: c3617fd]

- [x] Task: Read context — review `./spec.md` and `../../workflow.md` before starting this phase [c3617fd]
- [x] Task: Document "Transaction Wrapping" convention [c3617fd]
    - [x] Append a "Transaction Wrapping" section to `conductor/code_styleguides/sql.md` covering: when to use `db.transaction` (2+ writes), the `.returning()` pattern, post-commit advisory work isolation (try/catch after commit), and the gold-standard reference (`submitReviewHandler`) [c3617fd]
- [x] Task: Investigate fileKey trust gap (document only) [c3617fd]
