...
- [ ] Task: Scaffold TanStack Start project using `pnpm create @tanstack/start`
    - [ ] Run `pnpm create @tanstack/start . --template minimal` (or equivalent) to scaffold in project root
    - [ ] **Remove all boilerplate:** delete demo routes, template pages, icons, default assets, and any unnecessary files generated by the scaffold
    - [ ] Verify project root has clean structure (no nested project directory)
    - [ ] Create `package.json` with correct dependencies:
        - Runtime: `@tanstack/start`, `@tanstack/react-router`, `@tanstack/react-query`, `tailwindcss` v4, `zod`, `drizzle-orm`, `better-auth`, `typesafe-i18n`, `react-hook-form`, `@hookform/resolvers`
        - Dev: `typescript`, `vitest`, `@vitejs/plugin-react`, `husky`, `lint-staged`, `prettier`, `eslint`, `tsx`
    - [ ] Create `tsconfig.json` with path aliases (`@/` → `src/`), vitest types included
    - [ ] Install all dependencies via `pnpm install`
    - [ ] Create/adjust `app.config.ts` with TanStack Start configuration (routes dir, SSR settings)
    - [ ] Create/adjust `vite.config.ts` with React plugin, path aliases, and placeholder vitest config
    - [ ] Verify `pnpm run dev` starts without errors (ignore missing routes for now)
- [ ] Task: Configure TypeScript, ESLint, and Prettier
    - [ ] Create `eslint.config.js` with TypeScript + React rules, `@/` alias resolution, import ordering
    - [ ] Create `.prettierrc` (single quotes, trailing commas, 100 print width, compatible with ESLint)
    - [ ] Verify ESLint and Prettier work on a minimal file (no errors, no conflicts)
