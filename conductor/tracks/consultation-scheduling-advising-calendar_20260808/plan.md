...
- [x] **Phase 7 Verification & Checkpoint**
   - [x] Run the exact focused instructor UI test command and announce it before execution.
   - [x] Manually test slot publish, student booking visibility, reschedule, cancel, complete/no-show, and conflict feedback.
   - [x] Verify responsive behavior, dark mode, keyboard use, and destructive-action confirmation.
   - [x] Obtain confirmation, attach the verification note, record the checkpoint SHA, and commit the plan update.
...
- [x] **Task 8.1: Full lifecycle E2E coverage**
   - [x] Add Playwright coverage for instructor publish → student book → notification → reschedule/cancel → completion/no-show → explicit consultation record → instructor verification.
   - [x] Add cross-role authorization and calendar-feed scenarios.
   - [x] Add a DST/timezone scenario and mobile viewport coverage.
   - [x] Confirm RED behavior for newly added scenarios, then make them pass.
 ...
 - [x] **Task 8.2: Regression and quality gates**
    - [x] Run focused unit suites.
    - [x] Run `pnpm test`.
    - [x] Run `pnpm test:integration`.
    - [x] Run `pnpm test:coverage` and confirm new modules exceed 80%.
    - [x] Run `pnpm typecheck`.
    - [x] Run `pnpm lint`.
    - [x] Run `pnpm format` and inspect the resulting diff.
    - [x] Run `pnpm check:i18n`.
    - [x] Run relevant accessibility/E2E checks.
    - [x] Run `pnpm build`.
 ...
 - [x] **Task 8.3: Documentation and self-review**
   ...
   - [x] Record migration, rollback, scheduler/reminder, privacy, and operational notes.
   - [x] Review every acceptance criterion and out-of-scope item against the implementation.
   - [x] Confirm no new dependency or tech-stack deviation was introduced; if one is required, stop and update `tech-stack.md` before proceeding.
...
- [x] Task: Apply review suggestions
  - [x] Add regression coverage for student rescheduling ownership, reminder retries/failures, stale reminder candidates, slot overlap races, and soft-deleted participant privacy.
  - [x] Fix high-severity authorization and reminder reliability findings.
  - [x] Fix medium-severity stale candidate, slot overlap, and soft-deleted PII findings.
  - [x] Synchronize track metadata and resolve the exposed appointment-detail placeholder.
  - [x] Rerun focused tests and quality checks.
    - RED/GREEN: Added same-assignment cross-student rescheduling coverage; reminder claim-release, stale-candidate, failed-delivery, and failed-write coverage; concurrent slot-overlap and soft-deleted student PII integration coverage; appointment-detail role/ownership/error coverage (4/4).
    - Hardening: Student rescheduling now scopes the original appointment to the authenticated student while allowing an unbooked replacement; reminders use nullable sentAt plus claimedAt, locked candidate rechecks, stale-claim recovery, partial-delivery retry, and propagated scan failures; slot creation uses instructor advisory transaction locking and half-open overlap exclusion; instructor/student projections filter private/deleted participant data; status/student consistency is enforced by migration 0027; detail reads are implemented in appointments-detail.server.ts.
    - Verification: focused appointment suites 60/60; scanner coverage 98.7% statements, 83.78% branches, 100% functions, 100% lines; full serialized unit run 462 files/4,557 tests; serialized simak_test integration 16 files/44 tests; coverage 87.34% statements, 80.53% branches, 82.55% functions, 88.18% lines; typecheck, build, i18n, diff check, and lint passed (four pre-existing warnings); migrations 0026/0027 rollback/re-apply verified on simak_test.
    - Implementation commits: 8f20460d (`fix(track-058): address appointment review findings`) and 49c65acf (`test(track-058): cover appointment detail authorization`); verification git notes attached.
