...
- [x] Task: Add `common.goHome` i18n key and regenerate types
    - [x] Add `"goHome": "Go Home"` to `common` object in `locales/en.json`
    - [x] Add `"goHome": "Ke Beranda"` to `common` object in `locales/id.json`
    - [x] Run `pnpm generate:i18n` to regenerate `src/i18n/types.ts` and `src/i18n/detect-locale.ts`
    - [x] Run `pnpm check:i18n` to verify EN↔ID parity (714=714 keys, `goHome` unused warning expected until code uses it)

- [x] Task: Write failing tests for navigation and pagination fixes (Red Phase)
    - [x] Create/update test for `NotFoundComponent` in `__root.tsx`: assert link `href="/"` (not `/dashboard`) and label uses `t('common.goHome')`
    - [x] Create/update test for `RootErrorComponent` in `src/components/error-boundary.tsx`: assert label uses `t('common.goHome')`
    - [x] Create/update test for `UsersPage` in `src/routes/_authenticated/admin/users/index.tsx`: assert `<Pagination>` is NOT rendered when `users.length === 0`, and IS rendered when `users.length > 0`
    - [x] Run `pnpm test` and confirm the new tests fail as expected (4 failed, 16 passed)

- [x] Task: Implement navigation and pagination fixes (Green Phase)
    - [x] In `src/routes/__root.tsx` `NotFoundComponent`: change `href="/dashboard"` to `href="/"` and label from `t('common.goToDashboard')` to `t('common.goHome')`
    - [x] In `src/components/error-boundary.tsx` `RootErrorComponent`: change label from `t('common.goToDashboard')` to `t('common.goHome')` (link already goes to `/`)
    - [x] In `src/routes/_authenticated/admin/users/index.tsx`: wrap `<Pagination>` in `{users.length > 0 && (...)}`
    - [x] Run `pnpm test` and confirm all tests now pass (20/20 passed)

- [x] Task: Verify quality gates and commit [98dc0d6]
    - [x] Run `pnpm test:coverage` (≥80% on all thresholds) — 87.66% stmts / 81.32% branches / 81.36% funcs / 88.29% lines
    - [x] Run `pnpm typecheck`
    - [x] Run `pnpm lint` (including `simak-i18n/no-hardcoded` rule) — 0 warnings, 0 errors
    - [x] Run `pnpm check:i18n` — 713=713 keys, no unused keys
    - [x] Verify no broken links remain: `grep -r 'href="/dashboard"' src/` returns zero matches
    - [x] Stage changes and commit with message `fix(ui): Fix 404 dead link, ErrorBoundary label, and empty-list pagination (UX-38, UX-39, UX-57)`
    - [x] Attach git note with task summary to the commit
