...
| **PDF Rendering**   | PDFKit 0.19.x          | Server-only institutional PDF generation with embedded fonts and multi-page tables |
...
- **2026-08-09:** Selected PDFKit 0.19.x for TRACK-061 server-only institutional reports after evaluating PDFKit, pdf-lib, `@react-pdf/renderer`, and pdfmake. PDFKit meets the Node 20 ESM and Docker/Coolify baseline without Chromium or native runtime dependencies, supports embedded static TTF fonts and native multi-page tables, and keeps the renderer isolated from client bundles. Reports will bundle static Noto Sans Regular/Bold fonts, prohibit remote or user-selected resources, bound report content, and use semantic PDF tests plus a production-build smoke check. `pdf-lib` would require custom table pagination, React PDF adds a larger layout/reconciler surface without a native table primitive, and pdfmake has less suitable ESM ergonomics and a history of server-side resource-resolution advisories. The dependency is intentionally not installed until the rendering phase.
