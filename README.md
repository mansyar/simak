...
│   ├── components/       → React components (ui/, layout/, dashboard/, instructor/feedback-snippets/, etc.)
│   ├── server/           → Server functions (*.ts = stubs, *.server.ts = handlers), including instructor-owned feedback snippets
...
- **E2E tests** (`tests/e2e/`) — Playwright E2E tests covering critical user flows: auth route guards + invalid login, admin user management + edit/delete with reassignment, admin template CRUD, instructor assignment creation, private instructor feedback snippets (CRUD, archive/restore, ownership isolation, role access, and review-picker insertion), student file submission + upload UI validation + cross-student access denial, instructor review workflow + notification assertions, consultation lifecycle, extension workflow, password setup lifecycle, role dashboard smoke tests, settings hub, rubric grading, checkpoint discussions Q&A, cross-role lifecycle integration, mobile viewport, and axe accessibility scans. Runs on 3 browser projects: chromium (desktop), firefox (desktop), and mobile-chrome (Pixel 7). Uses a dedicated test database (`postgres-test` Docker service, port 5433). Run with `pnpm test:e2e` (headless) or `pnpm test:e2e:ui` (interactive). The global setup migrates, truncates, and seeds the test DB (including TRACK-049 active/archived/second-instructor snippet fixtures) before each run; each spec file resets the DB for isolation. The focused TRACK-049 spec passed 12/12 across all three projects; a full-suite run exceeded the available environment execution window without an aggregate result.
