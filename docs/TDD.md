...
| templateType   | text, not null     | `password_reset` \| `invitation` \| `sla_alert` \| `two_factor` \| `submission_received` \| `review_completed` \| `revision_requested` \| `consultation_verified` \| `consultation_rejected` \| `extension_approved` \| `extension_rejected` \| `extension_requested` \| `deadline_reminder` \| `student_at_risk` (14 values — 4 original + 10 event types; `deadline_reminder` added in TRACK-021, `student_at_risk` added in TRACK-023) |
