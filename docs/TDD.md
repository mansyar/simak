...
| **UI Library**     | shadcn/ui + Base UI primitives | Accessible, composable components with shared focus, keyboard, and ARIA behavior.                    |
...
### UI/UX audit remediation decisions (August 2026)

The archived `ui-ux-audit-remediation_20260804` track is the current implementation reference for the cross-role UI contract:

- Shared interactive controls use 44px minimum hit areas, visible focus, keyboard-safe mobile drawers, ARIA tabs/disclosures, mobile-safe dialogs/sheets, and mobile-priority table/card layouts.
- Loading, empty, not-found, authorization, and server-error states are distinct. Structured `ServerError` values are mapped through localized `ErrorCode` keys; raw server messages are not exposed to users. Mutation outcomes use localized status/alert live regions or toasts, with retry and input-preservation behavior where applicable.
- Student date surfaces use centralized, hydration-safe IANA timezone formatting. The persisted reduced-motion preference is reflected on `document.documentElement` and applied globally alongside the system preference media query.
- The nonce-based SSR/CSP path remains authoritative. `ThemeScript` suppresses the browser-masked nonce hydration mismatch without weakening CSP or router nonce propagation.
- Final verification passed 435 unit/coverage files with 4,323 tests, the full 104-test Chromium suite, typecheck, lint, formatting, i18n parity/unused-key checks, and focused axe/hydration checks. No serious/critical axe violations or hydration mismatches remained.
...
│   │   ├── appointments.ts   → Appointment slot, booking, cancellation, rescheduling, outcome, and detail stubs
│   │   ├── appointments.server.ts → Instructor slot creation/list/cancellation handlers with overlap locking
│   │   ├── appointments-lifecycle.server.ts → Student booking and lifecycle helpers
│   │   ├── appointments-cancellation.server.ts → Student-owned cancellation handler
│   │   ├── appointments-rescheduling.server.ts → Stable-identity replacement-slot rescheduling
│   │   ├── appointments-outcomes.server.ts → Instructor completion/no-show transitions
│   │   ├── appointments-detail.server.ts → Role-scoped appointment detail queries
│   │   ├── appointments-list.server.ts → Student available/history list handlers
...
│   │   ├── use-debounced-callback.ts → Cancelable debounce hook (setTimeout/clearTimeout, 300ms for remote search inputs)
...
All list views (assignments, reviews, users, notifications, consultations, submissions, templates, feedback snippets, extension requests) implement offset-based pagination:

- **20 items per page** as default page size (max 100 via Zod `max(100)` cap, with feedback snippets applying a stricter max of 50).
...
- **Loading state**: skeleton rows while the next page loads. Search-backed TanStack Query consumers retain the previous result while a committed query loads; prefetch next page on scroll near the bottom where applicable.
...
- **Error handling:** Dashboard and page-level data fetches distinguish loading, empty, not-found, authorization, and server-error states and provide localized retry actions. Structured `ServerError` values map through `ErrorCode` translations; raw server/database messages are not presented in user-facing UI. Side-data `useEffect` fetches (student assignment detail consultations/extensions) are wrapped in try/catch with a `sideDataError` state flag that renders an inline error banner with a retry button (using the `errors.fetchFailed` and `common.refresh` i18n keys). Auto-actions like `openForReview` on the review detail page are wrapped in try/catch — on failure, a localized `toast.error()` is shown and the self-navigation loop (`navigate({ replace: true })`) is prevented, keeping the user on the page. File upload errors in `CheckpointSubmissionPage` are differentiated: network failures (caught as `TypeError`) show `files.networkError` ("Network error, check your connection"), while server-side non-2xx responses show `files.serverError` ("Server error, try again") — replacing the former generic `files.uploadError` message.
...
- **Search debounce (TRACK-056):** Remote search inputs on admin users/templates/email queue/audit-log pages, instructor and student assignments, and feedback snippets maintain immediate local state and commit through the cancelable `useDebouncedCallback` hook (`src/hooks/use-debounced-callback.ts`) after 300ms. Prop synchronization preserves route state; clearing cancels pending work, commits an empty search immediately, and resets the page. TanStack Query consumers use stable query-key factories and `placeholderData` where applicable so previous results remain visible during committed-query loading. StudentPicker and TemplatePicker intentionally remain local-only: their fixed 200-student/100-template server queries are normalized and filtered with `useMemo`, and repeated selection checks use `Set` membership.
- **Search workload boundaries (TRACK-056):** Feedback snippet list/picker queries accept `page` and `limit` (default 20, maximum 50), return a minimal list projection plus total metadata, and preserve body text needed for insertion/editing. Template type discovery is a separate cached query, so template row searches do not repeat type aggregation. Email queue status summaries are a separate cached query, so row searches execute only data/count work. Users and assignment list handlers start independent data/count queries together; audit JSONB detail search explicitly casts `details` to text before applying the contains predicate.
...
**Consultation** — student-instructor meeting log, tied to a specific checkpoint; verified evidence remains separate from scheduled appointments.
**Appointment** — one-off assignment-linked advising slot with UTC start/end instants, optional checkpoint/student booking, and `available`/`booked`/`cancelled`/`completed`/`no_show` lifecycle.
**AppointmentReminder** — durable per-appointment, participant, and 24-hour/1-hour tier deduplication record.
Claims use `claimedAt` and nullable `sentAt`: stale claims are reclaimed, locked candidate rechecks prevent delivery after lifecycle changes, failed participant claims are released, and scanner failures propagate to the queue boundary for retry.
...
**NotificationPreference** — per-user, per-type, per-channel toggle stored in `users.settings` JSONB column (no separate table). Keyed by notification `type` string (e.g., `submission_received`), with `{ email?: boolean; inApp?: boolean }` values. Absent key or absent sub-field = default `true` (enabled). Established types are organized into 4 groups (Reviews, Consultations, Submissions, System); appointment lifecycle and reminder events use the same preference gate and default-enabled behavior. Security types (password_reset, invitation, two_factor, sla_alert) are exempt from email gating.
...
| templateType   | text, not null     | `password_reset` \| `invitation` \| `sla_alert` \| `two_factor` \| `submission_received` \| `review_completed` \| `revision_requested` \| `consultation_verified` \| `consultation_rejected` \| `extension_approved` \| `extension_rejected` \| `extension_requested` \| `deadline_reminder` \| `student_at_risk` \| `discussion_reply` \| `appointment_booked` \| `appointment_cancelled` \| `appointment_rescheduled` \| `appointment_completed` \| `appointment_no_show` \| `appointment_reminder_24h` \| `appointment_reminder_1h` (22 values — 4 original + 18 event types; appointment values added in TRACK-058) |
...
- Sent via Resend API. [v1] for all email types — auth-related emails (invitations, password reset, 2FA enable/disable), SLA alerts, and **18 event notification types** (the existing submission/review/revision/consultation/extension/deadline/risk/discussion events plus seven appointment lifecycle/reminder events). Event emails are dispatched as **post-commit advisory work** alongside existing in-app notifications via `enqueueEventEmail()` (`src/lib/event-email.ts`) and appointment-specific rendering in `src/lib/appointment-email.ts` — the primary operation always succeeds even if email enqueue fails. HTML templates escape user-derived content. Recipients with no verified email or soft-deleted accounts are silently skipped via `resolveEmailRecipient()` in `src/lib/email.ts`.
- **Localized email subjects:** Password reset, invitation, 2FA enable/disable, SLA alert, and all localized event notification subjects are resolved from i18n keys (`emails.subjects.*`) using the recipient's `locale` preference via `resolveEmailSubject()` in `src/lib/i18n-server.ts` (falls back to raw key if not found — non-crashing). Event email subjects are prefixed with `[SIMAK]` (e.g., `[SIMAK] New Submission Received`). Subjects support parameter interpolation (e.g., `{assignmentTitle}` in the deadline reminder subject) via optional `subjectParams` on `enqueueEventEmail`.
...
- **Risk-history daily processing (TRACK-062):** `processRiskHistoryJobs()` runs from the same tick loop on a 24-hour throttle. It records deterministic UTC-day snapshots for active assignment/student pairs in bounded batches, then anonymizes identifiable observations whose academic term ended more than five years ago. If academic context exists but no live checkpoint context remains, the recorder persists a low-risk empty-factor snapshot so passed, locked, or checkpoint-less assignments cannot wedge a batch. Anonymization clears lifecycle event/source details as well as identity and factor links; migration `0034_smart_quentin_quire.sql` updates the database consistency constraint for that irreversible state. Incomplete batches retry on the next tick; failures log `risk_history.daily_failed` and remain retryable, while a fully drained run logs `risk_history.daily_processed`. The job creates no notifications and never mutates grades, deadlines, checkpoint state, assignment access, or intervention state.
...
- **In-app preference gate:** `shouldSendInAppNotification(settings, type)` helper (`src/lib/notification-prefs.ts`) returns `false` only when `settings.notificationPrefs[type].inApp === false`. Applied at notification creation sites, including appointment lifecycle/reminder delivery (inline pattern for single-insert sites, batch filter for scanner/batch sites).
...
E2E tests run against a dedicated test database (`simak_test` on a separate `postgres-test` Docker service, port 5433) to avoid polluting the dev database. The global setup (`tests/e2e/global-setup.ts`) migrates the test DB, truncates all tables, and seeds test users (SuperAdmin, Admin, Instructor, Instructor2, Student, Student2, Student3 — all with `emailVerified: true`) plus an active academic term/course, primary section A, isolated negative-fixture section B, and role-aware memberships. Instructor2 is authorized only for section B; Student3 has an inactive membership in section B and is not enrolled in the primary assignment. The seed then creates an assignment template (3 checkpoints, Thesis, `minConsultations: 1`), an active individual assignment in section A with the first checkpoint unlocked (Student + Student2 enrolled), a pending consultation on the Proposal checkpoint, and TRACK-049 active, archived, and second-instructor-private feedback snippets. TRACK-054 revision-action-plan specs add their own ordered Revise-plan and resubmission fixtures. Each spec file resets the database (truncate + re-seed) via `resetDatabase()` before execution to ensure isolation.
...
| `instructor-assignments.spec.ts` | 3 | Create assignment from template in an authorized section, checkpoint state transitions (locked → unlocked → submitted), and 320px actionability |
| `academic-context.spec.ts` | 9 | Admin context/enrollment visibility, authorized instructor creation and lifecycle, cross-section/inactive-membership denial, clone/rollover history preservation, student context visibility, 320px layout, and admin axe checks |
