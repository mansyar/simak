...
- **Risk-history daily processing (TRACK-062):** `processRiskHistoryJobs()` runs from the same tick loop on a 24-hour throttle. It records deterministic UTC-day snapshots for active assignment/student pairs in bounded batches, then anonymizes identifiable observations whose academic term ended more than five years ago. Incomplete batches retry on the next tick; failures log `risk_history.daily_failed` and remain retryable, while a fully drained run logs `risk_history.daily_processed`. The job creates no notifications and never mutates grades, deadlines, checkpoint state, assignment access, or intervention state.
