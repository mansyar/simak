...
Migration `0024_powerful_clint_barton.sql` (TRACK-058) adds the UTC appointment table, lifecycle enum, assignment/checkpoint/participant foreign keys, duration/order checks, and participant/status indexes; migration `0025_breezy_carlie_cooper.sql` adds the `appointment_reminder_tier` enum and durable appointment reminder deduplication table with participant/tier indexes. Both migrations have companion rollback files and were forward-applied, inspected, rolled back, and re-applied against the disposable PostgreSQL test database.
