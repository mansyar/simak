...
- Users have a unified **Settings Hub** (role-specific routes at `/student/settings`, `/instructor/settings`, `/admin/settings`) where they can edit their profile name and avatar, change password, manage 2FA and sessions, switch language and theme, and configure accessibility preferences (reduced motion). Students can additionally set a validated IANA timezone and manage a private read-only iCalendar feed; these controls are not shown to instructors or admins (TRACK-055).
...
2. Sets password, logs in, and is redirected to the student dashboard with an always-visible prioritized Next Actions section above the active assignments overview, upcoming deadlines, pending reviews, and consultation reminders.
...
- **Feedback-to-revision action plans (TRACK-054):** Instructors may add an optional ordered plan of up to 10 plain-text items (500 characters each) to a `revise` review, with optional rubric-criterion links and immutable criterion-title snapshots. Students see the current plan on the checkpoint page and can reversibly mark items addressed; prior plans remain read-only history. Plans never block resubmission or replace the review decision, and unresolved current items enrich the existing Next Actions revise action without creating a second task system. The existing `revision_requested` notification is reused, while plan creation and status changes are audit-recorded without copying feedback text.
...
- **Consultation scheduling:** Instructors can publish bounded one-off 15–120-minute slots for active assignment context, and eligible students can book directly with an assignment-required, checkpoint-optional scope. Appointment booking, cancellation, rescheduling, completion, and no-show state are separate from consultation evidence; completing an appointment never verifies or creates a consultation automatically.
- Students and instructors see appointment times in the validated student timezone with UTC storage, preference-aware lifecycle notifications, deduplicated 24-hour/1-hour reminders, and private iCalendar events. Students can explicitly open the existing consultation form from a completed checkpoint-linked appointment with the checkpoint prefilled.
...
- **Appointment reminders:** A separate `processAppointmentReminders()` scan covers only future booked appointments in UTC 24-hour and 1-hour windows. `appointment_reminders` durably deduplicates by appointment, participant, and tier; nullable `sentAt` plus `claimedAt` support stale-claim cleanup, locked candidate rechecks, partial-delivery claim release, and retry after delivery or window failure. Cancelled, completed, and no-show appointments are excluded. Notification and email preferences apply independently, and scanner failures propagate to the queue boundary so the hourly attempt is retried without interrupting ordinary email processing.
...
- **Grade configuration:** Each assignment has an `assignment_grade_config` row (auto-created on assignment creation). Admins configure the grading scheme (`equal_weight`/`custom_weight`), custom per-checkpoint weights (must sum to 100), and letter grade bounds via a "Grade Settings" dialog on the gradebook page. All config changes are audit-logged. The TRACK-057 prelaunch migration refuses unexpected existing assignment rows rather than fabricating academic context; future production imports require an explicit legacy mapping.
...
- **Immutable provenance:** Migration `0028_flowery_cyclops.sql` adds nullable legacy course credits, versioned term-effective academic-record policies, and immutable `academic_records`; configured credits must be positive and unconfigured courses remain unavailable. Review migration `0029_left_boomerang.sql` enforces snapshot/student/assignment/release and section/course/term consistency and prevents transcript-source mutation after dependent records exist. Each record stores its source and policy context; releasing a later version inserts a new row and preserves prior history.
...
- **GPA rules:** Term GPA is the weighted mean of eligible complete records in the selected term. Cumulative GPA selects the latest eligible attempt per course using deterministic source-release-version, publication-time, and record-ID ordering. Aggregate summaries are calculated in PostgreSQL without loading every matching institutional record. All attempts remain visible; incomplete and withdrawn records remain visible but are excluded from GPA.
- **Role-scoped views:** Students use `/student/academic-records` for their own transcript and summaries; instructors use `/instructor/academic-records` only while actively enrolled as an instructor in the relevant section; admins and superadmins use `/admin/academic-records` for authorized records and provenance. Publication of an incomplete result requires an explicit, reasoned per-student outcome rather than inference from mutable working-grade state.
...
### Full-surface UI/UX remediation (August 2026)

- The archived `ui-ux-audit-remediation_20260804` track established the current UI contract across public/authentication, shared navigation, student, instructor, admin, settings, notifications, uploads/imports, and review workflows.
- Shared controls use 44px minimum hit areas, visible focus, keyboard-safe drawers/tabs/disclosures, semantic table captions/headers, distinct loading/empty/error states, localized retry and mutation feedback, and mobile-priority layouts from 320px upward.
- Structured server errors map through localized `ErrorCode` translations; raw server/database messages are not presented in user-facing UI. Student date surfaces use centralized, hydration-safe timezone formatting, and the persisted reduced-motion preference applies to the document root and global transitions.
- Final verification recorded 4,323 passing unit tests across 435 files, 104/104 Chromium E2E tests, no hydration mismatch, and no serious/critical axe violations. The track specification and implementation plan are retained under `conductor/archive/ui-ux-audit-remediation_20260804/`.

## TRACK-057 Implementation Outcome

The Academic Context & Semester Foundation is complete and archived. The implementation adds normalized terms, courses, sections, and role-aware enrollments; associates every assignment with exactly one section; makes assignment mode and lifecycle explicit; and applies active-context authorization to instructor, student, dashboard, calendar, submission, review, consultation, extension, intervention, and revision workflows.

Clone and semester rollover are transactional and auditable. They accept active or archived individual sources, reject draft/group sources, and create independent draft configuration with optional explicit students and fresh checkpoint state. Student participation, submissions, reviews, historical checkpoint state, released grades, and source audit history are never copied.

Migration `0023_sloppy_anthem.sql` and its rollback establish the schema with a prelaunch preflight that stops when existing assignments would require fabricated academic facts. Non-production reset/seed fixtures are deterministic; a future production import must provide an explicit legacy mapping. Bilingual admin, instructor, and student surfaces, accessibility checks, unit/integration/E2E coverage, and the documented quality gates are complete.

## UI Requirements

- **Design System**: "Warm Academic" design system with warm neutrals, serif display fonts (Fraunces for headings, DM Sans for body), and semantic color coding. The active tokens are defined in `src/app/global.css` and shared UI primitives; no separate `docs/UI_REDESIGN.md` file is required.
- **Responsive Design**: Usable on desktop and mobile devices with touch-friendly interactions (320px–1920px viewports). User-facing data-heavy surfaces use mobile-priority card or block-row layouts where appropriate: the instructor `ProgressTable` renders per-student cards below the `sm` breakpoint, `CheckpointListEditor` checkpoint rows stack with `flex-col sm:flex-row`, and the `AssignmentWizard` surfaces the current step name above the form on mobile. Admin users, audit-log, email-queue, and import previews expose readable mobile rows/cards with horizontal overflow retained only as an explicit fallback.
...
- **Student Next Actions (TRACK-053)**: The student dashboard places an always-visible bilingual action section above the existing widgets. It deterministically prioritizes unlocked submissions, revisions, required consultations, and upcoming deadlines; shows at most five primary actions; and groups all submitted/under-review work into waiting summaries with at most three representative links. For a current Revise action, unresolved revision-plan items are shown as concise context (up to three display items); addressed items are filtered without changing action priority or waiting behavior. Actions use existing authorized checkpoint or assignment destinations and do not create a separate persisted task system or request.
...
- **Accessibility**: Keyboard navigation, screen reader support. WCAG 2.1 AA compliance (contrast, focus, ARIA). Touch targets minimum 44×44px. Progress bars expose `role="progressbar"` + value attributes; collapsible sections expose `aria-expanded`/`aria-controls`; decorative timeline elements are `aria-hidden`; icon-only buttons (e.g. file download) have `aria-label`; dates render through the centralized `useStudentDateFormatter` hook for locale-aware, hydration-safe student timezone output. Every page has exactly one `<main>` landmark with `id="main-content"` and `tabIndex={-1}` — the skip-to-content link targets this element for keyboard focus management. All interactive content is contained within landmarks — `KeyboardCheatSheet` is rendered inside the `<header>` landmark, and the sonner `<Toaster>` exposes an `aria-label` for screen reader accessibility. Heading hierarchy follows a logical order with no level skips (h1 → h2 → h3) across all pages. Automated accessibility scanning is integrated into the E2E test suite via `@axe-core/playwright`; the complete Chromium verification suite and focused axe checks report zero serious/critical violations. The former moderate landmark, skip-link, region, and heading-order findings are remediated and regression-tested. Minor violations are documented in `docs/a11y-violations.md`.
...
- **Action Feedback**: All user-initiated mutations (create, update, delete, approve, reject, verify, unlock, extend, log consultation, profile/password changes) provide localized success and failure feedback through `MutationFeedback`, status/alert live regions, or the global toast system. A `showSuccessToast(message)` helper in `src/lib/toast.ts` mirrors `showErrorToast` for consistency. The global `<Toaster richColors position="top-right" />` in `__root.tsx` renders toasts — no per-call duration or position overrides. All feedback is localized via `t('key')` with keys in both `locales/en.json` and `locales/id.json`; no action completes silently.
...
- **Search Performance (TRACK-056)**: The search bar performance work preserves the debounced server-search contract while memoizing filtered client-side collections and stabilizing query-key inputs. Search results remain scoped to the active filters, clear actions reset immediately, and query invalidation does not trigger unnecessary duplicate requests.
...
- **AuditLog** — immutable record of all meaningful system actions (user created/deleted, template CRUD, assignment creation, review decisions, revision-action plan creation/status changes, deadline changes, unlock actions, consultation verifications/rejections, appointment lifecycle transitions, and grade release publication/withdrawal). Includes actor, action type, entity reference, and JSON details; revision-action and appointment entries omit private notes/reasons. Implemented with an admin viewer at `/admin/audit-log`.
...
- **CalendarFeedToken** — dedicated per-student credential record for the private read-only feed at `/api/calendar/ics`. Stores only a SHA-256 token hash, creation/revocation timestamps, and student ownership; a partial unique index permits at most one active token per student. The opaque bearer token is returned only when explicitly enabling or regenerating the feed and is never logged or stored in plaintext. The feed includes authorized future booked appointment events with stable appointment UIDs and UTC end times alongside deadline events; cancelled appointments are omitted.
- **Appointment** — one-off assignment-linked advising slot with instructor ownership, optional checkpoint/student booking, UTC start/end instants, five-state lifecycle (`available`, `booked`, `cancelled`, `completed`, `no_show`), transactional conflict checks, and immutable audit transitions. It is separate from `Consultation` evidence.
- **AppointmentReminder** — durable per-appointment, participant, and reminder-tier deduplication record for preference-aware 24-hour/1-hour reminder delivery. `claimedAt` supports stale-claim recovery while nullable `sentAt` distinguishes claimed work from successfully delivered work so failed deliveries can retry.
