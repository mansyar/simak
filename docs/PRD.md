...
**Search responsiveness decision (TRACK-056, August 2026):** All remote search
surfaces provide immediate local input feedback and commit a single request after a
300ms pause. Clearing is immediate and cancels pending work; newer searches cannot be
overwritten by stale responses, and existing results remain visible while a committed
search loads. Feedback snippets are paginated and bounded, while template type
discovery and email queue summaries are fetched independently from the searched rows.
Contains-search behavior is preserved with PostgreSQL `pg_trgm` GIN indexes. Student
and template pickers remain local-only and use bounded, memoized collections.
...
- **Search Debounce & Clear (TRACK-056)**: Admin users, templates, email queue, and audit-log searches; instructor and student assignment searches; and feedback-snippet list/picker searches use immediate local input with a 300ms cancelable remote commit. Typing 9 characters fires one settled request rather than one request per character. Clear buttons cancel pending work, clear immediately, and reset the page. Prior results remain visible during committed-query loading, and stale results cannot replace a newer search. StudentPicker and TemplatePicker remain local-only, bounded filters with no per-keystroke server fetch.
