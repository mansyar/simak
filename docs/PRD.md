...
- **AuditLog** — immutable record of all meaningful system actions (user created/deleted, template CRUD, assignment creation, review decisions, revision-action plan creation/status changes, deadline changes, unlock actions, consultation verifications/rejections, and grade release publication/withdrawal). Includes actor, action type, entity reference, and JSON details; revision-action entries omit full feedback text. Implemented with an admin viewer at `/admin/audit-log`.
