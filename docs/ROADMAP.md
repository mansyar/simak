...
| File | Purpose |
|---|---|---|
| `package.json` | Package manager: `pnpm`. Scripts: `"prepare": "husky"` (auto-activates git hooks on install). Dependencies: TanStack Start, Router, Query, Tailwind v4, shadcn/ui, Zod, Drizzle, Better-Auth, typesafe-i18n, Vitest, Playwright. Dev deps: Husky, lint-staged, prettier, ESLint |
| `pnpm-lock.yaml` | Lock file (auto-generated by `pnpm install`) |
| `pnpm-workspace.yaml` | Workspace config (if monorepo needed later; single-project for now) |
...
| `playwright.config.ts` | Playwright config: test dir (`tests/e2e`), webServer (auto-start dev server), browser targets (chromium, firefox) |
...
| `src/app/global.css` | **Tailwind v4 entry point:** `@import "tailwindcss"`. Plus CSS custom properties for light/dark theme (colors for primary, background, foreground, muted, border, ring, etc.). shadcn/ui CSS variables structure |
...
| `src/components/ui/` | shadcn/ui primitives installed via CLI (Button, Input, Card, Avatar, Badge, Dialog, Select, Table, Tabs, Toast, etc. — added incrementally per track) |
| `components.json` | shadcn/ui configuration: `style: "default"`, `tailwind.css` path, `aliases`, `baseColor: "zinc"` |
| `docker-compose.yml` | Local dev services: PostgreSQL (port 5432), PgAdmin (optional). Volumes for data persistence |
| `docker/Dockerfile` | Multi-stage build (builder + slim runner). Expose port 3000 |
| `.env.example` | All required env vars with placeholder values. `DATABASE_URL` points to `localhost:5432` for local dev |
| `.gitignore` | Must ignore: `node_modules/`, `.env` (not `.env.example`), `.output/`, `dist/`, `build/`, `.lfstorage/`, `.husky/_/`, `coverage/`, `playwright-report/`, `test-results/`, `*.tsbuildinfo`, `.turbo/` |
| `.husky/pre-commit` | Run `pnpm lint-staged` on every commit |
| `.husky/pre-push` | Run `pnpm typecheck && pnpm vitest run --coverage && pnpm playwright test` on every push |
| `lint-staged.config.js` | Run on staged files: `eslint --fix`, `prettier --write`, `tsc --noEmit` (incremental), modularity check |
| `scripts/check-modularity.ts` | Script that fails if any source file exceeds 500 lines of code. Run as part of lint-staged |
...
- A placeholder page renders at `localhost:3000` with Tailwind v4 styles applied (both light and dark mode)
- `docker compose up` starts PostgreSQL on port 5432, app can connect to it
...
- `pnpm exec playwright install` installs browser binaries
- i18n types are generated on `pnpm run dev` — `t('key')` is type-safe from the first component

**Acceptance Criteria:**
- [ ] `pnpm run dev` starts the dev server and hot-reloads on file changes
- [ ] A basic page renders with Tailwind utility classes working in both light and dark themes
- [ ] `src/config/env.ts` throws a descriptive error when `DATABASE_URL` is missing
- [ ] TypeScript compilation passes with no errors
- [ ] `eslint.config.js` loads and linting passes on a minimal file
- [ ] `.prettierrc` formatting matches ESLint rules (no conflict)
- [ ] `docker compose up -d` starts PostgreSQL and the app connects successfully
- [ ] `docker build -t simak .` completes successfully
- [ ] `.env.example` documents all required variables
- [ ] `pnpm lint-staged` runs on `git commit` and fails if lint/typecheck/modularity checks fail
- [ ] `git push` is blocked if typecheck, vitest coverage, or Playwright tests fail
- [ ] A source file exceeding 500 lines triggers a pre-commit failure
- [ ] `shadcn add button` outputs files to `src/components/ui/` without errors
- [ ] `playwright.config.ts` is valid and `pnpm exec playwright test --list` shows available projects
- [ ] `pnpm run build` succeeds end-to-end (build pipeline verified)
