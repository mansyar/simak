...
*   **Status:** `Pending`
*   **Dependencies:** None (complementary to existing consultation tracking — consultations are formal advising sessions with verification gating; discussions are lightweight async Q&A with no gating). Extends existing notification infrastructure (TRACK-022 notification preferences, TRACK-018 event email notifications).
*   **Estimated Effort:** 4 Days / 2 Sprint Loops

#### Context Anchors (Traceability)
*   **PRD Reference:** `docs/PRD.md#consultation-tracking-kartu-bimbingan` (consultation tracking — the formal counterpart to discussions; discussions fill the informal Q&A gap), `docs/PRD.md#notifications` (notification system — extension point for `discussion_reply` notification type), `docs/PRD.md#checkpoints--submissions` (checkpoint lifecycle — the context where discussions live)
*   **TDD Reference:** `docs/TDD.md` `checkpoints` table (`src/db/schema/assignments.ts:77` — `assignmentId`, `studentId`, `state`), `consultations` table (`src/db/schema/consultations.ts:17` — `checkpointId`, `studentId`, `status`, `notes` — the formal counterpart), `notifications` table (`src/db/schema/notifications.ts:13` — `type`, `titleKey`, `messageKey`, `params`, `metadata`), `assignment_students` join table (`src/db/schema/assignments.ts:56` — maps students to assignments for ownership verification); `src/server/consultations.server.ts` (consultation handlers — pattern reference for two-file split + ownership guards), `src/lib/event-email.ts:21` (`enqueueEventEmail` — extension point for discussion reply emails), `src/lib/email-templates.ts` (HTML template builders — extension point for `buildDiscussionReplyHtml`), `src/lib/email.ts:9-22` (`TemplateType` union — must add `'discussion_reply'` alongside the `email-queue.ts` enum array), `src/lib/email.ts:45-67` (`resolveEmailRecipient` — used by `enqueueEventEmail`), `src/components/notifications/NotificationCenter.tsx:15-36` (`GROUP_CONFIGS` — 4 groups, `discussion_reply` fits in `consultations` group alongside `consultation_verified`, `consultation_logged`, `consultation_rejected`), `src/components/notifications/notification-routes.ts:18` (`getNotificationRoute` — extension point for `case 'discussion_reply'`), `src/components/notifications/NotificationItem.tsx:20` (`TYPE_ICONS` map — add `discussion_reply: MessageCircle`), `src/lib/query-keys.ts` (typed query-key factory — add `discussionKeys`), `src/routes/_authenticated/student/assignments/$id.checkpoints.$checkpointId.tsx` (student checkpoint detail page — mounting point for discussion panel), `src/routes/_authenticated/instructor/assignments/$id.tsx` (instructor assignment detail — mounting point for discussion view)
*   **Product Spec Reference:** `conductor/product.md` Track 6.1: Consultation Logging & Verification — consultations are formal, verified, and gated; discussions are informal, instant, and ungated. The two serve different communication needs.

#### Track Tech Stack
*   Drizzle ORM — New `checkpoint_discussions` table. Migration generated via `pnpm db:generate`.
*   New server function split: `src/server/discussions.ts` (client-safe Zod schemas + `createServerFn` stubs with `.inputValidator(Schema).handler(...)` builder pattern) + `src/server/discussions.server.ts` (handler implementations — `listDiscussionMessagesHandler`, `postDiscussionMessageHandler`, `deleteOwnMessageHandler`)
*   New shared module: `src/lib/discussion-email.ts` — `sendDiscussionReplyEmail()` helper (wraps `enqueueEventEmail` with `buildDiscussionReplyHtml`, matching `review-email.ts` pattern)
*   Existing infrastructure reuse: `enqueueEventEmail` (`src/lib/event-email.ts:21`), `resolveEmailRecipient` (`src/lib/email.ts:45-67`), `getNotificationKeys` (`src/lib/i18n-server.ts:44`), `shouldSendInAppNotification` + `maybeInsertNotification` (`src/lib/notification-prefs.ts`), `STRINGS` constant object + internal helpers in `email-templates.ts`, `getNotificationRoute` (`src/components/notifications/notification-routes.ts:18`), `TYPE_ICONS` map (`src/components/notifications/NotificationItem.tsx:20`), `GROUP_CONFIGS` (`src/components/notifications/NotificationCenter.tsx:15-36`)
*   `email_queue.templateType` is a Drizzle text enum (code-only change, no `ALTER TYPE`) — add `'discussion_reply'` to the enum array in `src/db/schema/email-queue.ts:10` AND to the `TemplateType` union in `src/lib/email.ts:9-22` (both must be updated — `enqueueEventEmail` takes `templateType: TemplateType`)
*   shadcn/ui components — `ScrollArea` for message list, `Textarea` for message input, `Avatar` for user initials, `Button` for send/delete, `Card` for message bubbles, `EmptyState` (requires `icon: React.ElementType` prop, e.g. `MessageCircle` from lucide-react)
*   TanStack Query — `useQuery` for message list (30s `refetchInterval` for near-real-time), `useMutation` for post/delete with optimistic updates (matching TRACK-014 pattern). New `discussionKeys` factory in `src/lib/query-keys.ts` (matching `notificationKeys`/`consultationKeys` pattern) for typed cache keys and invalidation.
*   Notification infrastructure extensions — add `discussion_reply` to `GROUP_CONFIGS` `consultations` group in `NotificationCenter.tsx`, add `case 'discussion_reply'` to `getNotificationRoute()` in `src/components/notifications/notification-routes.ts` (uses `metadata.target` to derive student vs instructor route), add `discussion_reply: MessageCircle` to `TYPE_ICONS` in `src/components/notifications/NotificationItem.tsx`
*   i18n codegen — new discussion keys in both `locales/en.json` and `locales/id.json` under `discussions.*` namespace

#### Scope Boundaries
*   **In Scope:**
    *   **Discussion schema:** New `checkpoint_discussions` table — `id` (serial PK), `checkpointId` (integer, FK → checkpoints, `onDelete: cascade`), `assignmentId` (integer, FK → assignments, `onDelete: cascade` — denormalized for efficient instructor queries without JOIN), `userId` (text, FK → users — the message author), `message` (text, not null, min 1 char, max 2000 chars), `parentMessageId` (integer, FK → `checkpoint_discussions.id`, nullable — for threaded replies), `createdAt` (timestamp), `updatedAt` (timestamp — set equal to `createdAt` on insert; never updated in v1, reserved for future edit support in v2). Indexes: `(checkpointId, createdAt ASC)` for message list queries, `(assignmentId, createdAt DESC)` for instructor overview, `(parentMessageId)` for reply threading. Soft-delete via `deletedAt` timestamp (matching codebase convention — used by `assignments`, `users`, `assignmentTemplates`, `templateCheckpoints`, `rubricCriteria`, `rubricLevels`). Deleted messages render as a muted "[deleted]" placeholder with no author or content.
    *   **Server functions (two-file split):** `src/server/discussions.ts` exports Zod schemas (`ListDiscussionMessagesSchema` with `checkpointId` + pagination, `PostDiscussionMessageSchema` with `checkpointId` + `message` + optional `parentMessageId`, `DeleteOwnMessageSchema` with `messageId`) + `createServerFn` stubs. `src/server/discussions.server.ts` exports handlers:
        ...
        *   `deleteOwnMessageHandler` — verifies the requesting user is the message author AND the message was posted within the deletion window (15 minutes). Soft-deletes the message via `deletedAt` timestamp (matching the codebase convention — used by `assignments`, `users`, `assignmentTemplates`, etc.). Replies to a deleted message are preserved (parent message shows "[deleted]" placeholder). Deleted messages render as a muted "[deleted]" placeholder with no author or content.
    *   **Student discussion UI:** New `DiscussionPanel` component on `/student/assignments/$id.checkpoints.$checkpointId.tsx` — below the submission/review section. `ScrollArea` with message bubbles (student messages left-aligned, instructor messages right-aligned, or vice versa — TBD by design). `Avatar` with user initials. `Textarea` + send `Button` (react-hook-form + Zod, `mode: 'onSubmit'`, matching existing form patterns). Message timestamps via `formatRelativeTime` (from `src/lib/format.ts`). Reply threading via `parentMessageId` (replies indented under parent). Delete button on own messages within 15-minute window. Empty state: "No messages yet — ask a question about this checkpoint" (reuses `EmptyState` component). Loading state: `Skeleton` placeholders. `useQuery` with 30s `refetchInterval` for near-real-time updates. `useMutation` with optimistic insert (matching TRACK-014 pattern — `onMutate` adds message to cache, `onError` rolls back).
    *   **Instructor discussion view:** New tab on `/instructor/assignments/$id` — "Discussions" tab (alongside Overview, Consultations, Extensions). Shows all discussion messages across all checkpoints for this assignment, grouped by checkpoint. Instructor can reply to any message. Same `DiscussionPanel` component reused with `instructorView` prop (changes message alignment, shows student name on each message). Linked from the review detail page as well (quick access to the checkpoint's discussion thread while reviewing).
    *   **Notification type:** New `discussion_reply` notification type — target: the other party (student → instructor, instructor → student). Route: `/student/assignments/{assignmentId}/checkpoints/{checkpointId}` (for student) or `/instructor/assignments/{assignmentId}` (for instructor). Route is determined by `metadata.target` (`'student'` | `'instructor'`) — the handler knows the recipient's role when creating the notification, so it includes `target` in metadata. `getNotificationRoute` in `src/components/notifications/notification-routes.ts` checks `metadata.target` to return the correct path. Params all strings: `authorName`, `checkpointName`, `assignmentTitle`, `messagePreview` (truncated to 100 chars). Added to `consultations` group in `GROUP_CONFIGS` (alongside `consultation_verified`, `consultation_logged`, `consultation_rejected` — all are checkpoint-scoped communication). Also add `discussion_reply: MessageCircle` to `TYPE_ICONS` in `NotificationItem.tsx` (otherwise falls back to generic `Bell` icon). Respects user notification preferences (TRACK-022) — can be disabled per-channel.
    *   **Email template:** New `buildDiscussionReplyHtml` in `src/lib/email-templates.ts` (named `build{Event}Html` matching convention) — uses internal helpers + `STRINGS` object for body content. Shows author name, checkpoint name, assignment title, message preview (truncated, HTML-escaped), CTA link to the checkpoint page. New email helper wrapper: `src/lib/discussion-email.ts` (`sendDiscussionReplyEmail` calling `enqueueEventEmail`, matching `review-email.ts` pattern). Email subject key: `emails.subjects.discussionReply` (camelCase).
    *   **Email queue enum extension:** Add `'discussion_reply'` to the `templateType` enum array in `src/db/schema/email-queue.ts:10` AND to the `TemplateType` union in `src/lib/email.ts:9-22` (both must be updated — `enqueueEventEmail` takes `templateType: TemplateType`). Both are code-only changes (Drizzle text enum, not pg enum, no `ALTER TYPE`).
    *   **i18n keys:** `discussions.title`, `discussions.placeholder`, `discussions.send`, `discussions.reply`, `discussions.delete`, `discussions.deleted`, `discussions.empty.title`/`.description`, `discussions.deleteWindowExpired`, `discussions.loading`, `notifications.events.discussion_reply.title`/`.message` (params: `authorName`, `checkpointName`, `assignmentTitle`, `messagePreview`), `emails.subjects.discussionReply`. Note: the Discussions tab label uses `discussions.title` (matching the existing pattern where `consultations.title` and `extensions.queueTitle` are used as tab labels — there is no `instructorAssignments.tabs.*` namespace). Added to both locales, run `pnpm generate:i18n`.
    *   **Unit tests:** `listDiscussionMessagesHandler` (paginated, ordered by createdAt, includes author name/role, ownership verified — student can't view other students' discussions, instructor can view all in their assignments). `postDiscussionMessageHandler` (validates message length 1–2000, rejects empty, rejects > 2000, verifies ownership, inserts message, fires notification to other party with `metadata.target` set to recipient's role, advisory try/catch doesn't throw, `parentMessageId` validated to same checkpoint). `deleteOwnMessageHandler` (verifies author, rejects after 15-min window, soft-deletes via `deletedAt`, preserves reply threading). Email template rendering (both locales). Notification route derivation for `discussion_reply` (student target → checkpoint page, instructor target → assignment page, missing target → null). `DiscussionPanel` component (renders messages, send button disabled on empty input, delete button visible within window, deleted messages show placeholder, empty state renders with `icon` prop, loading skeleton renders).
*   **Out of Scope:**
    *   Real-time WebSocket/SSE push (v2 — v1 uses 30s polling via TanStack Query `refetchInterval`, sufficient for async Q&A)
    ...
    *   Class-wide discussion threads visible to all students in an assignment (v1 is per-student-per-checkpoint — private between student and instructor; v2 could add a "class discussion" mode)
...
#### High-Level Execution Vectors
*   **Phase 1 (Schema & Server Functions):** Create `checkpoint_discussions` table in `src/db/schema/discussions.ts` with proper FKs (`checkpointId` → checkpoints `onDelete: cascade`, `assignmentId` → assignments `onDelete: cascade`, `userId` → users, `parentMessageId` → `checkpoint_discussions.id` self-referencing), indexes (`(checkpointId, createdAt ASC)`, `(assignmentId, createdAt DESC)`, `(parentMessageId)`), and `deletedAt` timestamp (soft-delete for reply threading preservation). `updatedAt` set equal to `createdAt` on insert (reserved for v2 edit support). Add to `src/db/schema/index.ts` re-exports + relations. Generate migration via `pnpm db:generate`. Add `'discussion_reply'` to `templateType` enum array in `src/db/schema/email-queue.ts:10` AND to `TemplateType` union in `src/lib/email.ts:9-22` (both must be updated — `enqueueEventEmail` takes `templateType: TemplateType`). Create `src/server/discussions.ts` (Zod schemas: `ListDiscussionMessagesSchema` with `checkpointId` + `page`/`limit` pagination, `PostDiscussionMessageSchema` with `checkpointId` + `message` (min 1, max 2000) + optional `parentMessageId`, `DeleteOwnMessageSchema` with `messageId`) + `createServerFn` stubs with `.inputValidator(Schema).handler(...)` pattern. Create `src/server/discussions.server.ts` with `listDiscussionMessagesHandler` (paginated, JOIN users for author name/role, ownership verified), `postDiscussionMessageHandler` (validates length, verifies ownership — student owns checkpoint OR instructor owns assignment, inserts message, fires `discussion_reply` notification to the other party via `maybeInsertNotification` with `metadata.target` set to recipient's role + `enqueueEventEmail` advisory post-commit), `deleteOwnMessageHandler` (verifies author + 15-min window, soft-deletes via `deletedAt`). Add `discussionKeys` factory to `src/lib/query-keys.ts` (matching `notificationKeys`/`consultationKeys` pattern). Write tests (all handlers: success + ownership + validation failure cases; notification fires to correct party with `metadata.target`; advisory try/catch; `parentMessageId` validated to same checkpoint; delete window enforcement). Verify: messages list paginated and ordered, ownership enforced, notifications fire to the other party with correct `target`, delete respects time window, soft-delete preserves threading.
*   **Phase 2 (UI Components & Notifications):** Create `DiscussionPanel` component (`src/components/discussions/discussion-panel.tsx`) — `ScrollArea` with message bubbles (author `Avatar` with initials, message text, `formatRelativeTime` timestamp, reply threading via indentation, delete button on own messages within 15-min window, deleted messages as muted "[deleted]" placeholder). `Textarea` + send `Button` (react-hook-form + Zod `mode: 'onSubmit'`, `FormField`/`FormItem`/`FormMessage` pattern matching existing forms). `useQuery` with `discussionKeys.list(checkpointId, page)` and 30s `refetchInterval` for message list. `useMutation` for post (optimistic insert via `onMutate`/`onError`/`onSettled` matching TRACK-014 pattern) and delete (optimistic soft-delete). `EmptyState` when no messages (requires `icon: MessageCircle` prop from lucide-react, `title` and `description` props). `Skeleton` placeholders during loading. Mount on student checkpoint detail page (`$id.checkpoints.$checkpointId.tsx`) below submission section. Add "Discussions" tab on instructor assignment detail page (`$id.tsx`) — add `{ id: 'discussions', label: t('discussions.title') }` to `tabList` array and conditional render block (matching the existing `overview`/`consultations`/`extensions` tab pattern). Shows all checkpoint discussions for the assignment, grouped by checkpoint. Add `discussion_reply` to `GROUP_CONFIGS` in `NotificationCenter.tsx` (`consultations` group — alongside `consultation_verified`, `consultation_logged`, `consultation_rejected`). Add `case 'discussion_reply':` to `getNotificationRoute()` in `src/components/notifications/notification-routes.ts` — returns `/student/assignments/${meta.assignmentId}/checkpoints/${meta.checkpointId}` when `meta.target === 'student'`, returns `/instructor/assignments/${meta.assignmentId}` when `meta.target === 'instructor'`, returns null otherwise. Add `discussion_reply: MessageCircle` to `TYPE_ICONS` in `src/components/notifications/NotificationItem.tsx`. Write tests (panel renders messages, send disabled on empty, delete visible within window, deleted placeholder renders, empty state renders with icon, loading skeleton renders, optimistic insert works, notification route derived correctly for both student and instructor targets). Verify: student sees discussion panel on checkpoint page, instructor sees discussions tab, messages render with correct alignment, replies thread correctly, delete works within window, notifications are clickable and navigate to correct page based on `target`.
*   **Phase 3 (Email, i18n & Polish):** Add `buildDiscussionReplyHtml` to `email-templates.ts` (internal helpers, `STRINGS[locale].discussionReply` for intro, message preview truncated + HTML-escaped, CTA link to `${BETTER_AUTH_URL}/student/assignments/${assignmentId}/checkpoints/${checkpointId}` or `${BETTER_AUTH_URL}/instructor/assignments/${assignmentId}`). Create `src/lib/discussion-email.ts` wrapper (`sendDiscussionReplyEmail` calling `enqueueEventEmail` — matching `review-email.ts` pattern: receives all data as params, no DB lookup needed since `postDiscussionMessageHandler` already has checkpoint/assignment/author context). Add i18n keys to both locales (`discussions.*`, `notifications.events.discussion_reply.*`, `emails.subjects.discussionReply`). Run `pnpm generate:i18n`. Write tests (email renders both locales, message preview truncated and escaped, i18n parity). Verify: email renders in both locales, message preview is truncated and HTML-escaped, `pnpm check:i18n` parity, all files under 500 lines, `pnpm typecheck`/`pnpm lint` clean.

#### Verification & Definition of Done (DoD)
*   [ ] **Manual Checkpoint:** Student opens `/student/assignments/$id/checkpoints/$checkpointId` → sees a discussion panel below the submission section. Student types a question and clicks send → message appears instantly (optimistic update). Instructor opens `/instructor/assignments/$id` → clicks "Discussions" tab → sees the student's question grouped by checkpoint. Instructor types a reply and sends → student receives an in-app notification ("Instructor replied to your question in 'Abstract'") and an email (if not disabled in preferences). Student clicks the notification → navigates to the checkpoint page with the discussion panel visible. Student tries to delete their own message after 15 minutes → delete button is gone (window expired). Student tries to delete an instructor's message → no delete button visible. A deleted message shows as "[deleted]" with replies preserved underneath. Instructor opens the review detail page for a submission → can access the checkpoint's discussion thread (quick link). A student cannot view another student's discussion thread (ownership guard). `discussion_reply` notifications are clickable and navigate to the correct page. Notifications respect user preferences (disabling in-app or email for `discussion_reply` stops that channel). `discussion_reply` appears in the "Consultations" group in the notification center.
*   [ ] **Automated Tests:** `pnpm test:unit` — all tests pass. New tests for: `listDiscussionMessagesHandler` (paginated 20/page, ordered by `createdAt ASC`, includes author name + role via JOIN, ownership verified — student can't view other students' discussions, instructor can view all in their assignments, soft-deleted messages excluded from list but replies to them included), `postDiscussionMessageHandler` (validates message length 1–2000, rejects empty, rejects > 2000, verifies ownership — student owns checkpoint OR instructor owns assignment, inserts message, fires `discussion_reply` notification to the other party via `maybeInsertNotification` with `metadata.target` set to recipient's role, fires email via `enqueueEventEmail` advisory post-commit try/catch, `parentMessageId` validated to same checkpoint), `deleteOwnMessageHandler` (verifies requesting user is author, rejects after 15-min deletion window, soft-deletes via `deletedAt`, preserves reply threading — replies to deleted message still render with "[deleted]" parent), `buildDiscussionReplyHtml` (both locales, `STRINGS` object, message preview truncated to 100 chars + HTML-escaped, CTA link correct), `getNotificationRoute` for `discussion_reply` (student target → checkpoint page, instructor target → assignment page, missing target → null), `DiscussionPanel` component (renders message list, send button disabled on empty input, delete button visible within 15-min window only on own messages, deleted messages show "[deleted]" placeholder, empty state renders `EmptyState` with `icon` prop, loading state renders `Skeleton`, optimistic insert on send, optimistic soft-delete on delete). `pnpm check:i18n` — parity for all new keys. `pnpm test:coverage` >= 80% on all thresholds.
*   [ ] **Conductor Review:** New `checkpoint_discussions` table with proper FKs (`checkpointId` → checkpoints `onDelete: cascade`, `assignmentId` → assignments `onDelete: cascade`, `userId` → users, `parentMessageId` self-referencing), indexes (`(checkpointId, createdAt ASC)`, `(assignmentId, createdAt DESC)`, `(parentMessageId)`), and `deletedAt` soft-delete (preserves reply threading — deleted messages render as "[deleted]" placeholder, not removed). `updatedAt` set equal to `createdAt` on insert (reserved for v2 edit support, never updated in v1). Server functions follow two-file split with `.inputValidator(Schema).handler(...)` builder pattern. Ownership guards on all handlers (student owns checkpoint OR instructor owns assignment). Notification fires to the correct party (student → instructor, instructor → student) via `maybeInsertNotification` (respects TRACK-022 preferences) + `enqueueEventEmail` (advisory, post-commit, try/catch). `discussion_reply` notification includes `metadata.target` (`'student'` | `'instructor'`) for route derivation. `getNotificationRoute` in `src/components/notifications/notification-routes.ts` checks `metadata.target` to return student or instructor route. `discussion_reply` in `consultations` group of `GROUP_CONFIGS` (alongside `consultation_verified`, `consultation_logged`, `consultation_rejected`). `discussion_reply: MessageCircle` in `TYPE_ICONS` in `NotificationItem.tsx`. `discussionKeys` factory added to `src/lib/query-keys.ts`. `'discussion_reply'` added to both `templateType` enum array in `email-queue.ts` AND `TemplateType` union in `email.ts` (both required — `enqueueEventEmail` takes `templateType: TemplateType`). Email builder named `buildDiscussionReplyHtml` (matching convention). Email subject key `emails.subjects.discussionReply` (camelCase). Tab label uses `discussions.title` (matching existing pattern — no `instructorAssignments.tabs.*` namespace). `templateType` is Drizzle text enum (code-only change, no `ALTER TYPE`). 30s polling via TanStack Query `refetchInterval` (no WebSocket — v1 tradeoff for simplicity). Optimistic updates on post/delete matching TRACK-014 pattern. All files under 500 lines. `pnpm typecheck`, `pnpm lint`, `pnpm check:i18n` all clean.
...
└── TRACK-026: Checkpoint Discussion / Q&A Threads [no deps — extends notification infrastructure]
...
| **O** | TRACK-026 | Independent — new domain (discussions), extends notification infrastructure (TRACK-022 complete) and email queue (TRACK-018 complete). No file overlap with TRACK-025 (different domain: discussions vs grading). Minor overlap with TRACK-022 on notification type registry and `GROUP_CONFIGS` — coordinate if parallelized |
