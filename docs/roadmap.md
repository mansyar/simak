...
- **Status:** 📋 Proposed · **Audit IDs:** None (explicit TRACK-051/PRD deferral) · **Deps:** TRACK-057 (course/term/enrollment context), TRACK-051 (immutable released grades)
- **Estimated Effort:** ~20–35 single-developer days (provisional; institutional grading-policy discovery required)
- **Context anchors:** `docs/PRD.md` — Gradebook & Final Grade Computation and Data Model Summary; `conductor/archive/grade-release-workflow_20260802/` — formal transcripts and GPA are out of scope; `docs/TDD.md` — `assignment_grade_config`, `final_grades`, and `grade_release_snapshots`; current implementation in `src/db/schema/gradebook.ts`
...
- **Decisions:** Use only eligible, published `grade_release_snapshots` as the student-facing official source; never aggregate provisional `final_grades` into an official transcript. Define course credits, grade-to-point mapping, rounding, repeated-course handling, withdrawal/incomplete states, replacement versus inclusion rules, and effective term before schema work. Prefer immutable release-derived records or a deterministic read model with a rebuild path; transcript history must not change when live grade computation is recomputed. PDF generation belongs to TRACK-061, not this track.
- **Scope:** Academic-record contracts tied to TRACK-057 courses/terms/enrollments; transcript and term-summary queries; configurable institutional GPA policy; student transcript view; authorized staff/instructor views as approved; audit-safe handling of released, withdrawn, incomplete, and repeated results; CSV/structured export contracts for the reporting track; and regression coverage for grade release, reassignment, soft deletion, and historical snapshots.
...
- **Execution vectors:** Define the academic policy as pure tested functions before persistence; join only through authorized course/term enrollment and active published releases; add explicit transaction boundaries for record publication/rebuild; preserve prior release versions and audit events; avoid recalculating historical values from mutable rubric/config rows; expose structured server responses through the standard split; and add representative fixtures for retakes, incomplete work, withdrawn releases, missing grades, and mixed grading schemes.
- **Definition of Done:** Authorized students and staff can view a deterministic transcript and term GPA based only on approved released grades; policy edge cases are documented and tested; later grade recomputation cannot rewrite historical records; course/term/enrollment and assignment authorization are enforced; exports provide a stable contract for reporting; bilingual accessible UI and all repository quality gates pass.
...
├── TRACK-060: Academic Records — Transcript & GPA [📋 Proposed — depends on 057, 051]
