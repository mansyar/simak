...
- **Status:** `Complete (Archived)` (archived to `conductor/archive/analytics-reporting_20260722/`)
...
- **Decisions:**
  ...
  - **Review fix — CSV formula injection (CWE-1236, Medium):** During code review, `escapeCsvValue` was hardened to prefix cells starting with `=`, `+`, `-`, `@`, TAB, or CR with a single quote (`'`), preventing CSV injection when exported files are opened in spreadsheet applications.
  - **Review fix — audit-log date validation (Low):** `ExportAuditLogCsvSchema` changed from `z.string()` to `z.coerce.date().optional()` so date params are parsed and validated as real dates rather than raw strings.
  - **Review fix — ownership consistency (Low):** Added an assignment-ownership check to `exportReviewHistoryCsvHandler` (mirroring `exportStudentProgressCsvHandler`) — returns `NOT_FOUND` if the requesting instructor does not own the assignment.
...
- **Phase 1 (Admin Analytics):** Created `src/server/analytics-admin.server.ts` + `src/server/analytics.ts` (admin-only `getAdminAnalyticsData` — 8 parallel aggregate queries via `Promise.all`: consultation verification rate, deadline breach rate, assignment status distribution by state, submission/review volume by date via `date_trunc`, reviews completed count, DAU/WAU). Accepts `{ range: '7d' | '30d' | '90d' | 'all' }` + optional custom start/end. Created `/admin/analytics` route with URL search param parsed by route loader. `MetricCard` grid + data tables for trend data. Added admin sidebar entry. Commit: `7925e4d`.
- **Phase 2 (Instructor Analytics):** Created `src/server/analytics-instructor.server.ts` + `getInstructorAnalyticsData` stub (instructor-scoped — reviews completed, avg response time via `EXTRACT(EPOCH FROM reviewedAt - uploadedAt)`, SLA breach count where `> 259200` (3 days), students supervised, assignments active). Created `/instructor/analytics` route with URL search param. Added instructor sidebar entry. Commit: `d1fec37`.
- **Phase 3 (CSV Export):** Created `src/server/analytics-export.server.ts` + 5 CSV handlers (`exportUsersCsv`, `exportAuditLogCsv`, `exportAssignmentProgressCsv` for admin; `exportStudentProgressCsv`, `exportReviewHistoryCsv` for instructor — both with ownership checks). Each returns a CSV string; client creates `Blob` + triggers download via `src/lib/download.ts` + `useCsvDownload` hook. Added "Export CSV" buttons to admin users, audit log, and instructor assignment detail. Commit: `f5cc59a`.
- **Phase 4 (Excel Export):** Added client-side SheetJS export (`src/lib/excel-export.ts`) on both analytics pages — reuses existing `xlsx` dependency. "Export Excel" button uses `xlsx.utils.book_new()` + `json_to_sheet()` + `write()`. Commit: `f9ca721`.
- **Phase 5 (i18n + Polish):** Added all new labels/headers in both `locales/en.json` and `locales/id.json` (`analytics.*` + `analyticsInstructor.*` namespaces). Ran `pnpm generate:i18n`. Verified `pnpm check:i18n` parity (799 keys EN/ID). Commit: `84c4979`.

#### Verification & Definition of Done (DoD)

- [x] **Manual Checkpoint:** Admin opens `/admin/analytics?range=30d` — sees NEW metrics (consultation verification rate, deadline breach rate, assignment status distribution, submission volume trend). Instructor opens `/instructor/analytics?range=30d` — sees personal response-time metrics. Change date range to `90d` — URL updates, data refreshes. Click "Export CSV" on the admin users page — downloads a valid CSV. Click "Export Excel" on the analytics page — downloads a valid `.xlsx`. Switch to Indonesian — all labels translate.
- [x] **Automated Tests:** `pnpm test:unit` — 2,982 tests pass across 301 files (2,944 + 38 xlsx-threaded). 10 new test files mirroring `src/` structure covering: `getAdminAnalyticsData`/`getInstructorAnalyticsData` (aggregate correctness with date-range filtering, role guards), 5 CSV export functions (format, headers, role guards, ownership checks, CSV injection mitigation), client-side Excel export generation. Coverage ≥80% (lines 87.55%, statements 81.38%, branches 81.37%, functions 88.2%). Phase commits: `7925e4d`, `d1fec37`, `f5cc59a`, `f9ca721`, `84c4979`.
- [x] **Conductor Review:** Analytics server functions are admin/instructor-scoped (role guards). No new DB tables (Phase 1 aggregates only). CSV export via server function returning string + client Blob download (no streaming infrastructure). Excel export reuses existing `xlsx` dep (no new dep). 3-file handler split (`analytics-admin.server.ts`, `analytics-instructor.server.ts`, `analytics-export.server.ts`) each under 500 lines. URL search params for date range (`?range=30d`). New metrics do NOT duplicate existing dashboard metrics. New i18n keys in both locales (799 keys EN/ID parity confirmed). `pnpm typecheck` clean; `pnpm lint` 0 errors (2 pre-existing warnings — unused `data` params in export handlers); `pnpm check:i18n` clean. Review applied 3 fixes (CSV injection mitigation, audit-log date validation, ownership check) — commit `15389f2`. Track archived — commit `fdfd946`.
