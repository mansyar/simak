...
- **Status:** 📋 Planned · **Audit IDs:** None (proactive security hardening) · **Deps:** None
- **Problem:** `app.config.ts` (repo root, 7 lines — only `appDirectory: 'src'`) emits zero HTTP security headers: no Content-Security-Policy, no X-Frame-Options, no X-Content-Type-Options, no Strict-Transport-Security, no Referrer-Policy. Traefik handles TLS termination but application-level security headers are absent. CSP is the primary browser-level defense against XSS — the app has rich user-generated content (assignment descriptions, review feedback, discussion Q&A) that could serve as stored-XSS vectors. Additionally, `__root.tsx` contains an inline `<script dangerouslySetInnerHTML>` (theme script, lines 43-62) for FOUC prevention, and TanStack Router's `<Scripts />` component (line 177) emits inline hydration scripts — both would be blocked by a naive `script-src 'self'` CSP directive.
- **Proposed solution:** Implement a **nonce-based CSP** using TanStack Start's built-in nonce support. Create a `createStart` instance (`src/start.ts`) with a global `createMiddleware` that: (1) generates a cryptographic nonce per request (`crypto.randomBytes(16).toString('base64')`), (2) sets all security headers via `setResponseHeader()` from `@tanstack/react-start/server`, (3) passes the nonce to the router context. Configure `ssr: { nonce }` on the router (`src/router.tsx`) so TanStack Start auto-attaches the nonce to `<script>` and `<style>` tags (including the theme script and hydration scripts). Use `Content-Security-Policy-Report-Only` in development (violations logged, not blocked) and `Content-Security-Policy` in production (enforced). The CSP directives: `default-src 'self'; script-src 'nonce-{nonce}' 'strict-dynamic'; style-src 'nonce-{nonce}'; img-src 'self' data: https:; connect-src 'self' <R2_ENDPOINT domain>; frame-src 'self'; frame-ancestors 'none'; base-uri 'self'; form-action 'self'; object-src 'none'; upgrade-insecure-requests`. The R2 presigned upload/download URLs are generated from the `R2_ENDPOINT` env var domain (`src/lib/storage.ts` → `getR2Client()`), so `connect-src` must include that domain. `R2_PUBLIC_URL` exists in the env schema but is unused in source code. Better Auth client (`authClient`) makes fetch calls to `/api/auth/*` (same-origin — covered by `'self'`). The DOCX preview `<iframe srcDoc={...} sandbox="">` in `ReviewFilePreview.tsx` is governed by `frame-src 'self'` (srcdoc frames are same-origin, `sandbox=""` prevents script execution). Also set: `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Strict-Transport-Security: max-age=31536000; includeSubDomains` (defense-in-depth behind Traefik), `Referrer-Policy: strict-origin-when-cross-origin`, `Permissions-Policy: geolocation=(), microphone=(), camera=()`.
- **Scope:** New `src/start.ts` (`createStart` instance with CSP middleware — generates nonce, sets all security headers, propagates nonce to context), `src/router.tsx` (add `ssr: { nonce }` to router config — TanStack Start auto-attaches nonces to script/style tags), new test file for middleware + header verification. Note: `__root.tsx` ThemeScript and `<Scripts />` do NOT need manual changes — TanStack Start's nonce mechanism auto-attaches to `dangerouslySetInnerHTML` scripts and hydration scripts when `ssr.nonce` is configured.
- **Execution vectors:** Create `createStart` instance with `createMiddleware().server()` that generates nonce + sets headers via `setResponseHeader()`; use `import.meta.env.DEV` to switch between Report-Only and enforce; configure `ssr: { nonce }` on router (nonce read from `getGlobalStartContext().contextAfterGlobalMiddlewares.nonce` on server, `document.querySelector('meta[property=csp-nonce]').content` on client); unit tests for middleware (nonce generation, header values, Report-Only vs enforce); E2E test asserting header presence + values on authenticated + unauthenticated routes; verify CSP doesn't break Tailwind v4 styles, R2 presigned URL PUT/GET, Better Auth fetch calls, DOCX iframe preview, or hydration scripts; document the CSP allowlist rationale + nonce lifecycle in `conductor/tech-stack.md`.
- **Definition of Done:** All 6 security headers present on all HTTP responses (including API + auth routes); nonce-based CSP allows framework scripts/styles (theme script, hydration scripts, Tailwind v4) via auto-attached nonces; `connect-src` includes `R2_ENDPOINT` domain; `Content-Security-Policy-Report-Only` in dev, `Content-Security-Policy` in prod; E2E test asserts header presence + values; all existing E2E tests pass (no CSP breakage — verify no Report-Only violations in dev); `src/start.ts` under 500 lines; typecheck + lint + test clean.
...
Milestone 12: Security, Reliability & Real-Time Infrastructure
├── TRACK-041: HTTP Security Headers [📋 Planned — no deps]
...
| **U** | TRACK-041, TRACK-042 (complete — archived), TRACK-045 | Fully independent — TRACK-041 touches `app.config.ts`; TRACK-042 touched `src/db/index.ts` + `env.ts` (complete — archived); TRACK-045 touches `src/router.tsx` + `email-queue-init.ts` + new `shutdown.ts`. No file overlap. All are config/infrastructure-only changes |
