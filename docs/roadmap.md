...
> **Purpose:** Global architectural index mapping the remediation path following a comprehensive three-way audit (bugs, performance, UX). Completed tracks are summarized as index entries with links to full specs in `conductor/archive/`. Proposed and active/planned tracks retain full detail until approved or archived.
...
- **Key decisions:** `EmptyState` component replaces `null` returns; `formatDistanceToNow` for relative dates; `SLABadge` `title` tooltip; mobile card layout for `ProgressTable`; `flex-col` responsive for `CheckpointListEditor`; UX-44 timezone support was deferred to and delivered by TRACK-055
...
> These tracks address security, reliability, and observability gaps identified in a post-completion review of the production-ready codebase following Milestone 11. All tracks are proactive infrastructure improvements — no new product features, except TRACK-046 (SSE notifications), which is retained as a later backlog candidate for the existing notification UX. Findings are not audit-driven; they were identified during a comprehensive codebase review covering configuration, database, server-function architecture, and background processing. Tracks are ordered by recommended priority: security hardening first (CSP headers, rate limiting), then reliability (DB pool config, graceful shutdown), and observability completeness (request ID wiring).
...
- **Deferral rationale:** The current Coolify deployment is a single instance, and the existing 30-second polling behavior is an accepted v1 contract in `docs/PRD.md` and the TRACK-026 discussion specification. SSE should be revisited after pilot telemetry or a demonstrated latency/multi-tab need; multi-instance delivery would require a shared broker rather than the proposed in-memory connection manager.

- **Status:** ⏸ Deferred backlog · **Audit IDs:** None (proactive UX enhancement — supersedes conscious v1 decision "No WebSocket/SSE in v1") · **Deps:** TRACK-022 (User Notification Preferences — provides `maybeInsertNotification` chokepoint in `src/lib/notification-prefs.ts`), TRACK-030 (NotificationCenter Infinite Query Migration — provides the client-side notification data layer via `useInfiniteQuery` + `notificationKeys` factory)
...
- **Status:** ✅ Complete — archived · **Audit IDs:** None (proactive student-support workflow enhancement) · **Deps:** TRACK-023 (at-risk student identification provides risk signals and dashboard context)
...
- **Decisions:** Intervention records are private operational records: they are visible only to the current instructor who owns the assignment, never to students or admins; the original actor is retained in the audit history. If an assignment is reassigned, the replacement instructor receives access as the new assignment owner and the former instructor loses access. Each record is scoped to one student and assignment, with at most one active intervention per pair (`open` or `monitoring`); resolved and dismissed records remain history and do not block a later intervention. Creation is available only when the live assessment contains at least one `student_inaction` factor; a `pending_review`-only risk cannot create a student intervention. Action types are fixed to `consultation`, `extension`, `discussion`, and `other`. The instructor explicitly manages status (`open`, `monitoring`, `resolved`, `dismissed`); `resolved` and `dismissed` are terminal and require a reason. Follow-up dates are displayed, including overdue state, in instructor views only; they do not create new in-app or email notifications. Risk scoring remains live and separate—an intervention never automatically resolves because risk factors disappear.
- **Scope:** An intervention data model with immutable audit events; instructor-only server functions and a dedicated `/instructor/interventions` page launched from the at-risk dashboard and assignment context; live student-inaction risk-factor context; action type, private note, status, optional follow-up date, and resolution/dismissal reason; filters and dashboard visibility for open and overdue follow-ups; reassignment-aware authorization. The assignment detail tab links existing consultation, extension, and discussion workflows without duplicating them.
...
- **Execution vectors:** Add the `interventions` schema, migration, partial active-pair unique index, and rollback; implement session/assignment ownership guards in the established server-function split; share live risk-context aggregation with the dashboard; add dashboard summary, dedicated management page, and assignment-context UI; link existing extension/consultation/discussion actions without duplicating them; add immutable audit events; preserve authorized handover during reassignment with transactional row locks; test privacy boundaries, student-inaction eligibility, pending-review-only rejection, duplicate-active prevention, reassignment access, terminal status transitions, and overdue display behavior.
- **Definition of Done:** The current owning instructor can create and manage interventions only for students in their assignments and only with a live student-inaction risk factor; a pending-review-only risk cannot create an intervention; only one active intervention exists per student-assignment pair while history is retained; risk factors are visible as context; resolution/dismissal requires a reason and creates an immutable audit event; assignment reassignment transfers record access to the replacement instructor; open and overdue follow-ups are visible in instructor views without sending notifications; students and admins cannot access individual records; unit/integration/E2E tests, typecheck, lint, i18n, accessibility, coverage, and build gates pass.
...
> This milestone improves the student loop without introducing the larger institutional hierarchy or external integrations. Student action prioritization and timezone/calendar planning are separate concerns: TRACK-053 establishes the action surface, TRACK-054 turns revision feedback into trackable work, and TRACK-055 adds student-local date presentation plus a read-only calendar feed.

### TRACK-053: Student Next Actions

- **Status:** ✅ Complete and archived in `conductor/archive/student-next-actions_20260802/` · **Audit IDs:** None (new product-enhancement scope) · **Deps:** None (coordinate with TRACK-013 date-display work, TRACK-021 deadline reminders, and TRACK-022 notification preferences)
- **Estimated Effort:** ~5–8 Days / 2–4 Sprint Loops
- **Problem:** The student dashboard currently presents four independent widgets — active assignments, upcoming deadlines, pending reviews, and consultation reminders. It shows status but does not answer “What should I do next?” across multiple assignments. The existing dashboard also limits deadline candidates before ranking them, hides unresolved reviews older than 30 days, and does not return checkpoint identifiers consistently enough for precise action links.
- **Decisions:** Derive actions from authoritative assignment, checkpoint, submission, review, consultation, and gating state; do not create a second persisted task system. Add the prioritized section above the existing widgets rather than replacing them. Show actionable states only: submit an unlocked checkpoint, review feedback and resubmit a `revise` checkpoint, or complete a consultation when required verified consultations are missing. Represent `submitted` and `under_review` items in a separate waiting summary. Use this deterministic priority order: overdue actionable checkpoints, revision checkpoints, required-consultation blockers, checkpoints due within the next absolute 168 hours, other dated actionable checkpoints, then undated actionable checkpoints; break ties by due date and stable identifiers. Show at most five primary actions and at most three representative waiting items, with additional work linked to the existing student assignments page. Generic discussion actions are excluded until an explicit actionable discussion state exists.
- **Scope:** A bilingual, accessible student-facing “Next Actions” section; an extended `getStudentDashboardDataHandler` response containing action-oriented DTOs with assignment/checkpoint/submission identifiers, state, due-date, and destination data; a compact grouped waiting summary; deterministic deduplication and priority resolution; and direct links to existing checkpoint/assignment surfaces. The resolver must load eligible incomplete checkpoints before applying the display limit, include all unresolved `submitted`/`under_review` items rather than retaining the current 30-day filter, and reuse existing authorization, checkpoint-transition, submission, review, and consultation-gating rules.
- **Out of Scope:** Timezone preferences, iCalendar feeds, Google Calendar or Microsoft OAuth, two-way calendar writes, consultation booking, a new persisted task table, a dedicated Next Actions page, generic discussion prompts, notification or reminder changes, analytics instrumentation, and structured revision-action storage from TRACK-054.
- **Execution vectors:** Extend `getStudentDashboardDataHandler` rather than adding a second client request; add the missing checkpoint identifiers and state needed for precise links; query all eligible incomplete work before truncating the ranked result; include both submitted and under-review waiting states; implement stable priority/tie-breaking and deduplication; preserve the existing assignment-page expansion path; add unit and dashboard regression tests for every state, priority, stale review, consultation-gating, authorization, and cross-user case; and verify mobile layouts, accessibility, and both locales.
- **Execution result:** Added the pure `student-next-actions` resolver and extended the existing student dashboard handler without a second request or persisted task system. The implementation loads complete authorized candidates before applying five-primary and three-waiting-link caps, resolves deterministic one-action-per-checkpoint priorities, preserves stale waiting summaries, and renders bilingual accessible UI with precise checkpoint/assignment links. Full unit and coverage gates passed with 4,089 tests, 87.92% statements, 80.96% branches, 83.91% functions, and 88.60% lines; typecheck, lint, i18n, and build passed. Focused Next Actions Playwright coverage passed on Chromium, Firefox, and mobile Chrome. The historical full-suite limitation recorded here was later superseded by the August 2026 full-surface UI/UX remediation, which passed 104 Chromium tests without hydration mismatch or serious/critical axe violations.
- **Definition of Done:** A student can identify the highest-value next action without opening every assignment; the new section appears above the existing dashboard widgets; only actionable work is presented as an action; waiting states appear as grouped counts with no more than three representative links; overdue, revision, consultation, next-168-hour, dated, and undated priorities are deterministic; stale unresolved reviews do not disappear; all links target authorized existing surfaces; existing reminders, checkpoint gating, grade visibility, and dashboard widgets remain unchanged; and unit/regression, focused E2E/accessibility/mobile, typecheck, lint, i18n, coverage, and build gates pass, with the full-suite E2E limitation documented.

### TRACK-054: Feedback-to-Revision Action Plans

- **Status:** ✅ Complete and archived in `conductor/archive/feedback-to-revision-action-plans_20260803/` · **Audit IDs:** None (student outcome and revision workflow enhancement) · **Deps:** TRACK-020 (rubric snapshots), TRACK-053 (Next Actions integration); coordinated with TRACK-049 (review-form feedback context)
- **Estimated Effort:** ~12–18 Days / 5–8 Sprint Loops
- **Problem:** The review loop records a pass/revise decision, free-text comments, optional feedback files, revision deadlines, rubric scores, and review history. Feedback snippets improve instructor speed, but neither free-text feedback nor rubric comments becomes a clear, trackable revision plan. Students can therefore resubmit without a shared view of which issues were addressed, while instructors must reconstruct prior feedback during the next review. The existing repeated-revise risk signal indicates the cost of this gap.
- **Decisions:** Action plans are optional on `revise` reviews and are not accepted for `pass` decisions; existing clients can continue submitting comment-only or feedback-file-only revise reviews. Existing comments, feedback files, rubric scoring, checkpoint state transitions, and resubmission behavior remain backward compatible. Action items are ordered plain text, limited to 10 items of 500 characters each, optionally linked to a rubric criterion, and store a criterion/title snapshot so later rubric edits cannot rewrite historical guidance. Item text and ordering become immutable when the review is submitted. Students may mark or unmark an item addressed until a newer revise plan supersedes it; instructors can view status but do not confirm or reopen items. Action items do not hard-block resubmission, automatically pass a checkpoint, or replace the authoritative review decision. The newest revise plan is current; older plans remain immutable history and are not copied or merged. Unresolved items enrich the TRACK-053 revise action only; after resubmission, the checkpoint is represented as waiting until the next review rather than creating a second task product.
- **Scope:** A revision-action data model and migration; instructor review-form controls for adding, removing, ordering, and editing items before submission; student checkpoint-page presentation of the current revision plan with reversible addressed state; secondary display of prior plans in review history; instructor review-detail visibility of current and prior action status; integration with the Next Actions resolver; reuse of the existing `revision_requested` notification without a new event type; audit records for plan creation and addressed-status changes; and bilingual, accessible UI.
- **Out of Scope:** AI-generated feedback, automatic grading or pass/revise recommendations, hard submission gates, rich-text or HTML feedback, shared action-plan templates, formal appeals, cross-assignment case files, and automatic intervention resolution.
- **Execution vectors:** Add a `revision_action_items` table and migration with review ownership, stable order, plain-text limits, optional rubric criterion foreign key, criterion/title snapshot, and addressed timestamp; extend review input validation with an optional maximum-10 item list; insert items atomically with the review and rubric-score transaction after existing validation, rejecting structured items on `pass`; enforce that linked criteria belong to the review checkpoint’s rubric; enforce instructor ownership and student assignment authorization on reads and status updates; allow addressed-status reversal only for the current plan before it is superseded; preserve historical items when a later review supersedes them without copying or merging; enrich the TRACK-053 revise action only and leave submitted/under-review checkpoints in the waiting summary; reuse the existing `revision_requested` notification without adding a new event; record plan creation and status changes in the audit log without duplicating full feedback text there; and add unit, integration, E2E, accessibility, and regression coverage for validation, transaction rollback, pass/revise, rubric/no-rubric, repeated revisions, resubmission, status updates, notifications, and authorization paths.
- **Execution result:** Added `revision_action_items` with migration `0021_round_mysterio.sql` and rollback; extended the review transaction with Revise-only validation, rubric criterion/title snapshots, atomic insertion, and existing `revision_requested` notification reuse; added instructor authoring and read-only history, student current/historical plan views with optimistic reversible addressed status, and current-plan enrichment for the existing Next Actions resolver. Current plans are selected deterministically by review timestamp plus review ID, comment-only follow-up reviews do not supersede an existing plan, and historical rows remain immutable. Added redacted audit records, role/ownership authorization, deterministic E2E reset handling, PostgreSQL integration coverage, Playwright Chromium/mobile and axe checks, and bilingual responsive UI. Isolated verification passed 423 files/4,201 tests with 87.80% statements, 80.93% branches, 83.79% functions, and 88.64% lines; typecheck, i18n, formatting, modularity, and build passed. Full integration retained two unrelated pre-existing failures in email-queue logger initialization and create-assignment deadline validation.
- **Definition of Done:** An instructor can optionally attach up to 10 actionable plain-text items to a revise decision without changing existing review semantics; pass decisions cannot create action items; item text and ordering remain stable after submission; a student can see the current plan and mark or unmark items addressed until a newer plan supersedes it; prior plans remain available in review history to the student and instructor; a later revise creates a new current plan without copying old items; unresolved current-plan items enrich the student Next Actions revise action, while resubmitted or under-review checkpoints remain waiting; the existing `revision_requested` notification continues to work without a new event type; audit records exist for plan creation and status changes; action plans cannot expose cross-user data or bypass checkpoint authorization; and all quality gates pass.

### TRACK-055: Student Timezone & iCalendar Support

- **Status:** ✅ Complete · **Audit IDs:** UX-44 (deferred from TRACK-013, delivered here) · **Deps:** TRACK-013, TRACK-053 · **Archived:** `conductor/archive/student-timezone-icalendar-support_20260803/`
- **Estimated Effort:** ~10–16 Days / 4–6 Sprint Loops
- **Problem:** Student deadline displays are locale-formatted but do not provide an explicit user timezone preference, and students cannot subscribe to SIMAK deadlines from an existing calendar. The current display fallback can differ between browser and server rendering, while the deadline reminder scanner must remain based on the authoritative stored deadline instant. Calendar clients also require a private subscription mechanism that does not depend on a browser session.
- **Decisions:** Limit timezone presentation changes to student-facing web deadline surfaces; the feed at `/api/calendar/ics` always emits the authoritative deadline instant as UTC and does not depend on the saved display timezone. Detect the browser IANA timezone after hydration, render a neutral placeholder before it is known, persist a valid detected or selected timezone through the existing `users.settings` JSONB boundary, and use UTC when detection is unavailable or invalid. Do not reinterpret or mutate stored deadlines; verify that existing deadline values are canonical UTC instants by convention before implementation. Provide a read-only personal iCalendar feed containing all non-passed checkpoint deadlines with a due date, including locked future checkpoints, plus an assignment final deadline when the assignment remains active and has any non-passed checkpoint, even if that checkpoint has no due date; overdue active work remains included. Selection verifies both checkpoint student ownership and `assignment_students` membership. Emit timed UTC events with stable UIDs and omit events on later refreshes when their checkpoint passes or assignment becomes inactive. Use a high-entropy opaque bearer credential stored only as a hash in a dedicated token table with one active token per student; create it only after explicit enablement, regenerate by invalidating the previous token, and support revocation. OAuth, two-way synchronization, and calendar writes remain deferred.
- **Scope:** Student timezone preference UI; explicit-timezone formatting for the student dashboard and assignment/checkpoint deadline surfaces; the private `/api/calendar/ics` endpoint; a token-table migration plus credential creation, regeneration, and revocation; stable event identity and deadline updates; RFC 5545-safe escaping/line endings; bounded rate limiting without client-forwarded IP trust, generic invalid-credential responses, sensitive-token log redaction, and safe cache/referrer headers; bilingual, accessible settings and feed-management states; and auditability for credential lifecycle events.
- **Out of Scope:** Instructor/admin timezone surfaces, email-template localization changes, changing stored deadline values or reminder semantics, Google Calendar or Microsoft OAuth, two-way calendar writes, consultation booking, recurring academic events, calendar invitations, and replacement of the existing deadline-reminder scanner or notification preferences.
- **Execution vectors:** Extend the settings schema/type and existing read-modify-write handler with an optional validated IANA timezone; detect and persist the browser timezone after hydration with a neutral pre-detection state and tested UTC fallback; extend `formatDate` or introduce a narrowly scoped timezone-aware formatter with SSR/client consistency and DST tests; verify existing deadline writes represent canonical UTC instants; add a migration and dedicated token table with one-active-token constraints, hashed opaque credentials, explicit enablement, regeneration, revocation, and lifecycle audit events; implement the student-only route-level feed endpoint with student ownership checks; select non-passed checkpoint deadlines and active-assignment final deadlines from authoritative data; emit timed UTC `VEVENT` records with stable UIDs, correct escaping, and safe cache headers; omit passed/inactive events on refresh; rate-limit the public credential route and prevent sensitive token logging; add settings, route, feed-format, authorization, token-lifecycle, revocation, DST, fallback, storage-invariant, and calendar-client compatibility tests; and verify mobile layouts and both locales.
...
### UI/UX Audit Remediation (August 2026)
...
- **Status:** ✅ Complete and archived in `conductor/archive/academic-context-semester-foundation_20260806/` · **Audit IDs:** None (post-MVP product expansion) · **Deps:** None for the foundation; consumed by TRACK-059, TRACK-060, TRACK-061, and TRACK-062
- **Effort:** Implemented across Phases 1–6 plus review fixes; current contracts are documented in `docs/PRD.md`, `docs/TDD.md`, and the archived Conductor plan
- **Context anchors:** `docs/PRD.md` — Academic Context Foundation; `docs/TDD.md` — academic tables, assignment context, migration, and E2E contracts; implementation in `src/db/schema/academic-context.ts` and `src/server/academic-context.ts`
- **Problem:** SIMAK currently models an assignment as a template-based workflow owned by one instructor and assigned directly to one or more students. `assignments` has no course, academic term, cohort/section, or explicit lifecycle state; `assignment_students` is the participation boundary, and every `checkpoints` row is per-student. This is sufficient for the current assignment workflow but does not provide the academic context required for semester rollover, group membership, transcripts, GPA, or institution-level reporting.
- **Decisions:** Use normalized academic terms, reusable courses, term-specific sections, and role-aware enrollments; no separate cohort subsystem. Keep templates reusable across terms. Each assignment belongs to one section and has explicit `individual`/`group` mode, with individual behavior supported and group behavior deferred. Use `draft` → `active` → `archived` as a history-safe lifecycle separate from `deletedAt`. Clone/rollover creates new configuration without copying students or historical workflow/release data. The prelaunch migration refuses unexpected existing assignments rather than fabricating academic facts; future production import requires an explicit legacy mapping.
- **Scope:** Academic terms and course/section records; role-aware enrollment relationships for students and instructors; assignment association with one section; active/archived lifecycle; instructor/admin authorization; assignment clone and controlled semester rollover; migration preflight/rollback and deterministic reset/seed policy; audit events for context/lifecycle/rollover operations; context filters on assignment and dashboard surfaces; and compatibility with existing `assignment_students`, per-student checkpoints, deadline rules, grade release, notifications, and calendar feeds.
- **Out of Scope:** Collaborative group membership or group submissions (TRACK-059); transcript/GPA computation (TRACK-060); SIS/LMS/OAuth integrations; attendance, fees, room scheduling, degree audits, and a complete course-catalog replacement; changes to the existing sequential checkpoint state machine.
- **Execution vectors:** Implement the normalized Drizzle schema and constraints; extend assignment creation/list/detail handlers and route loaders with ownership and enrollment checks; preserve the two-file server-function split and explicit projections; enforce a prelaunch migration preflight with a matching rollback and non-production reset/seed fixtures; make archive/clone/rollover transactional and auditable; add justified context indexes; update bilingual forms, filters, empty/error states, and E2E fixtures; and verify that existing individual assignments retain their current authorization, deadline, submission, review, and release behavior.
- **Current implementation outcome:** The foundation has normalized term/course/section/enrollment tables, explicit assignment context/mode/lifecycle fields, admin CRUD/update/archive and enrollment UI, authorized instructor creation/list/detail/lifecycle/clone/rollover flows, active-context student/dashboard/calendar/workflow access, deterministic reset/seed fixtures including cross-section and inactive-membership negatives, and cross-role Playwright coverage. Review fixes hardened typed admin errors, archived context handling, joined-row locking, and active term/section filters. Final quality gates and Principal Engineer review passed; downstream TRACK-059–062 work must consume the section/enrollment contracts rather than reintroducing a separate cohort model.
- **Definition of Done:** Admins can manage the approved academic context and authorized instructors can create, archive, clone, and roll assignments within it; unexpected prelaunch assignment rows stop migration instead of receiving fabricated context; rollover never mutates historical work or released grade snapshots; lifecycle writes enforce ownership, transaction, audit, and stale-state rules; student/instructor views show only authorized active academic contexts; individual assignment flows remain backward compatible; unit, integration, E2E, accessibility, i18n, typecheck, lint, modularity, coverage, and build gates pass.

### TRACK-058: Consultation Scheduling & Advising Calendar

- **Status:** ✅ Complete, reviewed, and archived in `conductor/archive/consultation-scheduling-advising-calendar_20260808/` · **Audit IDs:** None (post-MVP product expansion) · **Deps:** TRACK-055 (complete timezone/calendar primitives); coordinated with TRACK-057 and TRACK-018/022 for academic context and notification delivery
- **Estimated Effort:** ~15–25 single-developer days (provisional; conflict and reminder policy discovery required)
- **Context anchors:** `docs/PRD.md` — Consultation Tracking; `docs/TDD.md` — consultations and appointments; implementation in `src/db/schema/appointments.ts`, `src/server/appointments*.ts`, and the student/instructor appointment panels; TRACK-055's private feed was extended without changing its token boundary
- **Problem:** The current consultation domain records a student-entered session tied to a checkpoint with `pending`, `verified`, or `rejected` status. It has no availability, appointment time, booking conflict check, cancellation/rescheduling lifecycle, or calendar event. Students and instructors can document advising after it happens, but cannot use SIMAK to coordinate it.
- **Decisions:** Preserve existing consultation logs and their verified-count gating semantics. Model scheduled appointments as a separate record so booking state cannot be mistaken for verified consultation evidence. Use one-off 15–120-minute instructor slots, assignment-required/checkpoint-optional scope, direct student booking, canonical UTC instants, validated student timezone display, and transactional conflict checks. Reminder delivery respects notification preferences and remains advisory; cancellation, rescheduling, completion, and no-show never silently create or remove consultation evidence.
- **Scope:** One-off instructor slots; direct student booking; instructor/student cancellation and replacement-slot rescheduling; completion and no-show states; assignment/checkpoint linkage; timezone-aware display; conflict and ownership validation; localized in-app/email lifecycle notifications; durable 24-hour/1-hour reminders; audit events; private iCalendar appointment events alongside deadlines; and mobile-friendly bilingual student/instructor appointment views.
- **Out of Scope:** Google/Microsoft OAuth, two-way calendar writes, room/resource scheduling, recurring institutional calendars, group advising sessions, external counselor integrations, attendance management, and replacement of the existing consultation verification workflow.
- **Execution vectors:** Add appointment and reminder schemas with forward/rollback migrations; use the established client-safe/server-only split; protect booking/reschedule/cancel/outcome transitions with transaction and lock/re-check patterns; serialize instructor slot overlap checks with advisory locks; reuse `users.settings` timezone handling and notification preferences; deliver localized lifecycle email/in-app events through the existing queue; scan UTC reminder windows with durable participant/tier claims, stale-claim recovery, locked candidate rechecks, failed-claim release, and queue-level retry propagation; extend the private feed with stable appointment UIDs and UTC `DTEND`; keep verified consultation counts sourced only from verified evidence; and cover timezones, DST, overlap races, cancellation, soft-deleted users, privacy, accessibility, E2E, and bilingual UI.
- **Execution result:** Implemented migrations `0024_powerful_clint_barton.sql` through `0027_daily_synch.sql` with companion rollbacks; race-safe instructor/student appointment handlers with ownership, overlap, deleted-user privacy, status-consistency, and appointment-detail hardening; preference-aware lifecycle email/in-app notifications and retryable 24-hour/1-hour reminder scanning; private calendar appointment selection and UTC `DTEND` serialization; responsive student/instructor scheduling surfaces with explicit consultation prefill; and PostgreSQL, Playwright Chromium/mobile, accessibility-oriented, i18n, and regression coverage. Full unit verification passed 462 files and 4,557 tests; serialized disposable-database integration verification passed 16 files and 44 tests; coverage passed at 87.34% statements, 80.53% branches, 82.55% functions, and 88.18% lines; typecheck, lint, formatting, and build gates passed. Final Conductor review found no remaining findings, and the track was archived. The configured legacy database's pre-TRACK-057 migration history remains a documented operational limitation.
- **Definition of Done:** Students can request or book an available advising time, instructors can manage conflicts and lifecycle changes, and completed evidence can be recorded without changing existing verification rules; all participants see localized timezone-aware times; unauthorized users cannot discover appointments; reminders and calendar output are safe and preference-aware; audit history is complete; existing consultation gating remains unchanged; and unit, integration, E2E, accessibility, i18n, typecheck, lint, modularity, coverage, and build gates pass.

### TRACK-059: Collaborative Group Assignments

- **Status:** 📋 Proposed · **Audit IDs:** None (explicit PRD `[v2]` feature) · **Deps:** TRACK-057 (academic context and enrollment foundation), TRACK-025/051 (grade computation and release semantics)
- **Estimated Effort:** ~25–40 single-developer days (provisional; grading and membership policy discovery required)
- **Context anchors:** `docs/PRD.md` — Assignment Management and Data Model Summary (`AssignmentGroupMember` `[v2]`); `docs/TDD.md` — `assignment_students`, `checkpoints`, and `submissions`; current implementation in `src/db/schema/assignments.ts` and submission/review handlers
- **Problem:** The current model assumes one independent student workflow per assignment: `assignment_students` represents individual participation, `checkpoints` stores `studentId`, and checkpoint submissions/reviews/grades follow that identity. Group assignments are explicitly deferred, so adding a membership table alone would not define who owns checkpoint state, who may submit, how revisions work, or whether grades are shared or individual.
- **Decisions:** Make assignment mode (`individual` or `group`) explicit at creation and preserve existing individual assignments unchanged. Before implementation, approve the group semantics for checkpoint state, submission ownership, resubmission, membership changes, consultation requirements, review visibility, and grading. The recommended first slice is a shared group checkpoint workflow with the submitting member retained as `uploadedBy`, immutable membership/audit history, and a shared released grade; individual adjustments and peer grading should remain separate decisions. Membership changes must never rewrite historical submissions, reviews, or released snapshots.
- **Scope:** Group and group-member data model; instructor/admin group creation and membership management; student group visibility; group-aware assignment, checkpoint, submission, review, discussion, consultation, notification, risk, gradebook, and grade-release authorization; membership change audit trail; group progress and deadline views; migration-safe coexistence with individual assignments; and E2E coverage for cross-member access and unauthorized users.
- **Out of Scope:** Real-time co-editing, shared document editing, peer grading, chat replacement, external collaboration tools, automatic group formation based on optimization criteria, group advising-room scheduling, and retrofitting existing individual assignments into groups without an explicit migration.
- **Execution vectors:** Extend the schema without weakening current student ownership guards; define a group identity boundary before changing handler queries; update checkpoint/submission/review and grade computation projections together rather than layering group exceptions at the UI; use transactional membership changes with locked assignment/group rows; ensure notification and risk targets are group-aware; preserve `uploadedBy` for auditability; add fixture factories for both modes; and test all current checkpoint state transitions with group and individual assignments.
- **Definition of Done:** Instructors can create and manage group assignments, students can see only their own group work, authorized members can submit and follow the shared checkpoint lifecycle, reviews and revisions target the intended group, gradebook/release behavior follows the approved group policy, membership changes are audited and history-safe, individual assignments behave exactly as before, and all quality gates plus cross-role E2E and concurrency tests pass.

### TRACK-060: Academic Records — Transcript & GPA

- **Status:** ✅ Implemented — pending Conductor review · **Audit IDs:** None (implemented from the explicit TRACK-051/PRD deferral) · **Deps:** TRACK-057 (course/term/enrollment context), TRACK-051 (immutable released grades)
- **Estimated Effort:** ~20–35 single-developer days (provisional; institutional grading-policy discovery required)
- **Context anchors:** `conductor/tracks/academic-records-transcript-gpa_20260808/`; `docs/PRD.md` — Gradebook & Final Grade Computation and Data Model Summary; `conductor/archive/grade-release-workflow_20260802/` — formal transcripts and GPA were previously out of scope; `docs/TDD.md` — academic-context, grade-release, and academic-record contracts; current implementation in `src/db/schema/academic-records.ts`, `src/server/academic-records.ts`, and `src/server/academic-records-extras.server.ts`
- **Problem:** SIMAK now computes final grades and exposes an explicit draft/published release boundary, but the result is still assignment-scoped. There is no course credit, academic term, enrollment history, transcript view, cumulative GPA, or policy for repeats and incomplete grades. TRACK-051 deliberately excludes formal transcripts and GPA, so a new academic-context foundation is required before aggregation is trustworthy.
- **Decisions:** Use one explicitly designated assignment per section as the transcript source; create immutable records from eligible published snapshots inside the release transaction; retain versioned term-effective policies, course credits, source release data, and repeated attempts; use the latest eligible attempt for cumulative GPA while excluding incomplete/withdrawn outcomes; and keep PDF/scheduled delivery in TRACK-061.
- **Scope:** Academic-record contracts tied to TRACK-057 courses/terms/enrollments; immutable transcript and term-summary queries; configurable institutional GPA policy; student transcript view; authorized admin/instructor views; audit-safe handling of released, withdrawn, incomplete, and repeated results; and regression coverage for release, authorization, responsive UI, accessibility, and historical snapshots.
- **Out of Scope:** Grade appeals, registrar approval workflows, degree audits, program requirements, transfer credit, official signing/seals, external SIS/LMS integrations, provisional-grade student views, and PDF/scheduled report delivery.
- **Execution vectors:** Added migration `0028_flowery_cyclops.sql` with rollback and a preflight that refuses existing courses until credits can be supplied; implemented the pure policy engine before persistence; added transactional release/withdrawal persistence; exposed role-scoped typed server functions with active-version filtering; added bilingual responsive routes at `/student/academic-records`, `/instructor/academic-records`, and `/admin/academic-records`; and added deterministic Chromium/mobile E2E fixtures and accessibility checks.
- **Definition of Done:** Implementation is complete and pending Conductor review. Authorized students and staff can view deterministic transcript records and term/cumulative GPA based only on approved released grades; policy edge cases and immutable history are tested; later grade recomputation cannot rewrite academic records; course/term/enrollment and role authorization are enforced; bilingual accessible UI is covered; unit/integration coverage and project quality gates pass against the migrated disposable PostgreSQL database. The default development PostgreSQL instance still requires migration before integration use.
- **Operational notes:** `academic_records` are immutable release-derived rows keyed to source assignment/snapshot/release and policy versions. Only one active `isTranscriptSource` assignment is allowed per section. Deployments must apply migration `0028` after confirming the credits preflight; the E2E reset flow migrates and reseeds a disposable database. Student reads are self-scoped, instructor reads require active section enrollment, and admin provenance fields are restricted to admin views.

### TRACK-061: Institutional Reporting & Scheduled Delivery

- **Status:** 📋 Proposed · **Audit IDs:** None (explicit TRACK-019/PRD deferral) · **Deps:** TRACK-019 (existing analytics/export foundation), TRACK-018 (event email/queue foundation); coordinate with TRACK-057 and TRACK-060 for course, term, and transcript filters
- **Estimated Effort:** ~15–25 single-developer days (provisional; PDF and scheduler selection required)
- **Context anchors:** `docs/PRD.md` — Analytics & Reporting; `conductor/archive/analytics-reporting_20260722/` — CSV/Excel is complete while PDF and scheduled delivery are deferred; current implementation in `src/server/analytics.ts`, `src/server/analytics-export.server.ts`, and the in-process queue in `src/lib/email-queue-init.ts`
- **Problem:** TRACK-019 provides role-based admin/instructor analytics and on-demand CSV/Excel exports, but PDF export and scheduled/recurring delivery remain explicitly deferred. The current email queue tick is a processor, not a durable report scheduler, and generated reports must not bypass authorization or expose academic data through untracked attachments or links.
- **Decisions:** Start with a small allowlisted set of report templates based on existing analytics and later academic-context/transcript contracts; do not build arbitrary SQL or a general report designer. Select and document the scheduler boundary before implementation (Coolify-triggered job or a durable database-backed schedule); do not infer durable schedules from an in-memory interval. Report generation and delivery must have durable status, retry, audit, retention, and failure visibility, while the existing email queue remains the delivery mechanism where appropriate. Course/term filters and official transcript reports must use authoritative released data.
- **Scope:** On-demand PDF versions of approved admin/instructor reports; course/term/cohort filters; saved report parameters; one-off and recurring schedules; durable generation/delivery status; retry and failure handling; secure role-scoped output; email-queue integration; audit events; retention/cleanup; and bilingual accessible controls for creating, viewing, cancelling, and retrying reports.
- **Out of Scope:** Arbitrary report builders, BI/data-warehouse replication, external institutional delivery integrations, student analytics dashboards, risk-history storage (TRACK-062), raw database exports, and PDF rendering of every existing screen.
- **Execution vectors:** Reuse the existing analytics query contracts and CSV/Excel sanitization; isolate PDF rendering behind a server-only adapter; define whether generated artifacts are streamed, attached, or stored in R2 before implementation; add a durable report-job schema only if required by the selected scheduler; integrate post-commit email enqueue without making report generation part of the primary request transaction; enforce ownership and role filters server-side; and test large reports, retries, duplicate schedules, cancellation, expiry, formula/HTML safety, and locale output.
- **Definition of Done:** Authorized users can generate an approved report as PDF and schedule a one-off or recurring delivery; every job has observable pending/running/succeeded/failed/cancelled state and safe retry behavior; report contents respect academic context, release, ownership, and retention rules; failures are visible to operators; no raw academic data leaks through logs or unauthorized URLs; and unit, integration, E2E, accessibility, i18n, typecheck, lint, modularity, coverage, and build gates pass.
...
└── TRACK-046: Real-Time Notifications via SSE [⏸ Deferred backlog — depends on 022, 030]
...
└── TRACK-048: Backup & Restore Readiness [✅ Complete — archived — depends on 047]
...
├── TRACK-060: Academic Records — Transcript & GPA [✅ Implemented — pending review — depends on 057, 051]
├── TRACK-061: Institutional Reporting & Scheduled Delivery [📋 Proposed — depends on 019, 018; coordinate with 057, 060]
...
| **X** | TRACK-046 → TRACK-022 (complete), TRACK-030 (complete) | Deferred backlog candidate — TRACK-046 builds on notification infrastructure from TRACK-022 and the client-side data layer from TRACK-030. The proposed SSE route + in-memory connection manager remains unsuitable as a multi-instance design without a shared broker. |
| **Y** | TRACK-047 → TRACK-048 | Both tracks are complete and archived. TRACK-047 established the seven-copy Coolify pilot baseline; TRACK-048 documented the isolated restore evidence and non-implemented follow-up recommendations. |
...
| **AC** | TRACK-057 + TRACK-058 | TRACK-057 establishes the academic context consumed by later product tracks. TRACK-058 can begin from the existing consultation model and TRACK-055 timezone/calendar primitives, but should coordinate with the academic-context ownership and enrollment decisions. |
| **AD** | TRACK-059 + TRACK-060 | Both consume TRACK-057 but have distinct primary domains. Group assignments overlap grade/review/submission authorization, while transcripts overlap gradebook/release policy; coordinate the shared academic and grade semantics before parallel implementation. |
| **AE** | TRACK-061 → TRACK-057, TRACK-060 | Reporting can build its PDF/scheduling boundary from the existing analytics/export and email queue foundations, but authoritative course/term filters and transcript reports require the contracts from TRACK-057 and TRACK-060. |
| **AF** | TRACK-062 + TRACK-061 | Risk history and reporting have separate storage and authorization concerns but share analytics projections and privacy/retention decisions. Coordinate report contracts without coupling risk persistence to PDF delivery. |
...
| 20: Academic System Expansion (proposed) | 6 | ~120–195 Days (provisional) |
| **Total** | **62** | **~320–425 Days plus TRACK-056; includes six proposed tracks** |
...
New tracks follow a three-phase lifecycle in this document:

1. **Proposed** — Record a discussion candidate in the roadmap only. Cross-check its implementation boundaries and dependencies, but do not create `conductor/tracks/` files until the user approves the scope.
2. **Planned/Active** — After approval, retain a full-detail entry (status, audit IDs, deps, decisions, scope, execution vectors, DoD) and scaffold via `conductor_new_track`, which creates `conductor/tracks/<track_id>/spec.md` + `plan.md`.
3. **Complete** — On archival to `conductor/archive/<track_id>_<date>/`, collapse the entry to an index row: status badge, audit IDs, deps, one-line decision summary, archive link. The archive's `spec.md` and `plan.md` become the single source of truth for full detail.

This keeps the roadmap scannable while preserving an explicit discussion stage — proposed tracks are detailed enough to validate direction, but they are not implementation commitments until approved.
