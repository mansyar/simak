...
## Milestone 20: Academic System Expansion (Proposed Next Six)

> These six tracks are proposed from the current roadmap, PRD, TDD, and implementation boundaries. They are not yet approved Conductor tracks; no `conductor/tracks/` specifications or plans are created by this roadmap update. TRACK-046 remains a deferred infrastructure backlog item, while TRACK-048 is complete because Coolify already provides the pilot database backup and isolated restore baseline.

### TRACK-057: Academic Context & Semester Foundation

- **Status:** 🚧 Implementation in progress via `conductor/tracks/academic-context-semester-foundation_20260806/` · **Audit IDs:** None (post-MVP product expansion) · **Deps:** None for the foundation; consumed by TRACK-059, TRACK-060, TRACK-061, and TRACK-062
- **Estimated Effort:** ~25–40 single-developer days (provisional; schema and migration discovery required)
- **Context anchors:** `docs/PRD.md` — Assignment Management and Data Model Summary; `docs/TDD.md` — `assignments`, `assignment_students`, and `checkpoints`; current implementation in `src/db/schema/assignments.ts` and `src/server/assignments.server.ts`
...
- **Decisions:** Treat course/term/cohort/enrollment context and assignment archive/clone/semester rollover as one tightly coupled foundation track, as agreed during roadmap discussion. Keep templates reusable across terms. Introduce an explicit academic assignment lifecycle rather than treating the existing `deletedAt` soft-delete as an archive, and make clone/rollover create new assignment records without mutating submissions, reviews, audit history, or released grades. Existing assignments must be backfilled without fabricating course or term facts; the specification must choose a safe legacy-context strategy before migration. Keep individual assignment behavior as the default and make the individual/group mode explicit for future group work.
- **Scope:** Academic terms and course/section records; cohort/enrollment relationships for students and instructors; assignment association with the relevant course/term/cohort; active/archived lifecycle; instructor/admin authorization; assignment clone and controlled semester rollover; migration/backfill strategy for existing assignments; audit events for lifecycle and rollover operations; context filters on assignment and dashboard surfaces; and compatibility with existing `assignment_students`, per-student checkpoints, deadline rules, grade release, notifications, and calendar feeds.
...
- **Execution vectors:** Add the smallest normalized academic-context schema and constraints to the Drizzle model; extend assignment creation/list/detail handlers and route loaders with ownership and enrollment checks; preserve the two-file server-function split and explicit projections; design an idempotent migration/backfill for current rows; make archive/clone/rollover transactional and auditable; add context-aware query indexes only after measuring access patterns; update bilingual forms, filters, empty/error states, and E2E fixtures; and verify that existing individual assignments retain their current authorization, deadline, submission, review, and release behavior.
- **Current implementation outcome:** The foundation now has normalized term/course/section/enrollment tables, explicit assignment context/mode/lifecycle fields, admin CRUD/archive and enrollment UI, authorized instructor creation/list/detail/lifecycle/clone/rollover flows, active-only student/dashboard/calendar/workflow access, deterministic reset/seed fixtures, and cross-role Playwright coverage. Final repository quality gates and review remain pending; downstream TRACK-059–062 work must consume the section/enrollment contracts rather than reintroducing a separate cohort model.
- **Definition of Done:** Admins can manage the approved academic context and authorized instructors can create, archive, clone, and roll assignments within it; existing assignments have a documented, lossless legacy mapping; rollover never mutates historical work or released grade snapshots; all lifecycle writes enforce ownership, transaction, audit, and stale-state rules; student/instructor views show only authorized academic contexts; individual assignment flows remain backward compatible; unit, integration, E2E, accessibility, i18n, typecheck, lint, modularity, coverage, and build gates pass.

### TRACK-058: Consultation Scheduling & Advising Calendar

- **Status:** 📋 Proposed · **Audit IDs:** None (post-MVP product expansion) · **Deps:** TRACK-055 (complete timezone/calendar primitives); coordinate with TRACK-057 and TRACK-018/022 for academic context and notification delivery
- **Estimated Effort:** ~15–25 single-developer days (provisional; conflict and reminder policy discovery required)
- **Context anchors:** `docs/PRD.md` — Consultation Tracking; `docs/TDD.md` — `consultations`; current implementation in `src/db/schema/consultations.ts` and `src/server/consultations.server.ts`; TRACK-055 explicitly leaves consultation booking out of scope
...
- **Decisions:** Preserve existing consultation logs and their verified-count gating semantics. Model a scheduled appointment as a separate record or an explicitly separated lifecycle so booking state cannot be mistaken for verified consultation evidence. Store appointment instants canonically in UTC, display them with the validated student timezone from TRACK-055, and use transactional conflict checks for instructor availability. Keep availability bounded and understandable rather than building a general resource-booking system. Reminder delivery must respect TRACK-022 preferences and remain advisory; cancellation and rescheduling must not silently create or remove a verified consultation log.
- **Scope:** Instructor availability or bounded appointment slots; student booking/request flow; instructor confirmation, cancellation, rescheduling, completion, and no-show states; assignment/checkpoint linkage; timezone-aware display; conflict and ownership validation; localized in-app/email reminders; audit events; optional read-only calendar representation after the existing feed contract is extended deliberately; and mobile-friendly student/instructor calendar views.
...
- **Execution vectors:** Start with a domain decision on appointment-versus-log storage; add schemas and server functions using the established client-safe/server-only split; protect booking/reschedule/cancel transitions with transaction and lock/re-check patterns; reuse `users.settings` timezone handling and notification preferences; add bounded event email templates through the existing queue; keep verified consultation counts sourced only from verified evidence; and cover timezones, DST, overlap races, cancellation, reassignment, soft-deleted users, privacy, accessibility, and bilingual UI.
...
Milestone 20: Academic System Expansion (Proposed Next Six)
├── TRACK-057: Academic Context & Semester Foundation [📋 Proposed — no deps for foundation; feeds 059, 060, 061, 062]
