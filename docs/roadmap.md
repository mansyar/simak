...
- **Phase 1 (N+1 Fixes):** Rewrite `listVerifiedCountsHandler` with a single `GROUP BY` query. Rewrite the three sequential-UPDATE loops (`calculateExtensionAdjustment`, `bulkExtendHandler`, `adjustDeadlinesForBreach`) as bulk `UPDATE ... WHERE order > targetCheckpoint.order`. Rewrite `dispatchSLABreachNotifications`: remove dead `channel:'email'` rows, batch in-app INSERT into single `db.insert(notifications).values([...])`, `Promise.allSettled` the `sendSLAAlertEmail` calls. Rewrite `bulk-import.server.ts` post-commit: `Promise.allSettled` invitation emails, batch audit INSERT. Write tests verifying identical results.
- **Phase 2 (Pagination):** Add page/limit Zod params + total count queries + client-side Pagination components to the 5 list handlers (consultations, pending consultations, submissions, template assignments, extension requests). Add `.limit(20)` safety cap to the 2 dashboard queries (activeAssignments, assignmentOverview). Write tests verifying pagination behavior.
- **Phase 3 (Over-fetch, Parallel & Query Rewrite):** Narrow `listNotificationsHandler` SELECT to specific columns, construct response objects explicitly (no `...item` spread). Remove redundant locale query — use `session.user.locale`. Parallelize `listTemplatesHandler` and `listInstructorAssignmentsHandler` independent queries with `Promise.all`. Rewrite the `listPendingReviewsHandler` correlated subquery as a LATERAL join. Move `getObjectContentLength` calls before `db.transaction()` in both handlers.
- **Phase 4 (Tests):** Verify all query results are identical before/after N+1 rewrites. Add tests for pagination limits and total counts. Run `EXPLAIN ANALYZE` on the LATERAL join query and the bulk UPDATEs to confirm improved plans.

#### Verification & Definition of Done (DoD)

- [ ] **Manual Checkpoint:** Load the student assignment detail page — `listVerifiedCountsHandler` makes 1 query instead of N. Load notification list — response payload is smaller (no `metadata`, no leaked raw columns). Load instructor review queue — LATERAL join query is efficient in EXPLAIN ANALYZE. Submit a checkpoint — R2 HEAD check occurs before transaction opens (no lock held during I/O). Navigate to paginated list pages — Pagination component renders correctly.
- [ ] **Automated Tests:** `pnpm test:unit` — all existing tests pass (no regressions). New tests verify: pagination limits and total counts for 5 list handlers, GROUP BY result parity for `listVerifiedCountsHandler`, bulk UPDATE result parity for 3 extension/SLA functions, concurrent sending in `dispatchSLABreachNotifications`, batched audit INSERT in bulk-import, explicit response construction in `listNotificationsHandler`, LATERAL join query results. `pnpm test:coverage` >= 80%.
- [ ] **Conductor Review:** No sequential per-row queries remain in server handlers (grep for `for (const` in `.server.ts` files). All 5 list handlers have page/limit Zod params + total count queries. Both dashboard queries have `.limit(20)`. `listNotificationsHandler` does not spread `...item` into the response. `getObjectContentLength` is called before `db.transaction()` in both `submitCheckpointHandler` and `submitReviewHandler`. PERF-36 is explicitly documented as deferred.
