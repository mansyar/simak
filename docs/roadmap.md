...
| **U** | TRACK-041, TRACK-042, TRACK-045 | Fully independent — TRACK-041 touches `app.config.ts`; TRACK-042 touches `src/db/index.ts` + `env.ts`; TRACK-045 touches `src/router.tsx` + `email-queue-init.ts` + new `shutdown.ts`. No file overlap. All are config/infrastructure-only changes |
