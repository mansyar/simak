...
- **Status:** ✅ Complete, reviewed, and archived in `conductor/archive/academic-records-transcript-gpa_20260808/` · **Audit IDs:** None (implemented from the explicit TRACK-051/PRD deferral) · **Deps:** TRACK-057, TRACK-051
...
- **Context anchors:** `conductor/archive/academic-records-transcript-gpa_20260808/`; `docs/PRD.md`; `docs/TDD.md`; implementation in `src/db/schema/academic-records.ts` and `src/server/academic-records*.server.ts`
...
- **Decisions:** Use one explicitly designated assignment per section as the transcript source; create immutable records from eligible published snapshots inside the release transaction; retain versioned term-effective policies, course credits, source release data, and repeated attempts; use the latest eligible attempt for cumulative GPA while excluding incomplete/withdrawn outcomes. TRACK-061 subsequently consumed this boundary for on-demand PDF transcripts; recurring delivery remains deferred.
...
- **Out of Scope:** Grade appeals, registrar approval workflows, degree audits, program requirements, transfer credit, official signing/seals, external SIS/LMS integrations, provisional-grade student views, and scheduled/recurring report delivery. On-demand PDF delivery was implemented separately by TRACK-061.
- **Execution result:** Added migrations `0028` and review-hardening `0029`, preserving nullable legacy credits while enforcing positive configured values, immutable cross-table provenance, and transcript-source stability. Publication now requires active instructor enrollment and explicit reasoned incomplete outcomes; source/policy allocation is serialized; deterministic active selection follows release version, publication time, and record ID; GPA summaries use bounded database aggregation; and role routes use accessible bilingual controls and announcements.
- **Verification:** Focused migration-chain, integrity, server, and component tests passed; unit verification passed 4,616 tests with two load-related route-test timeouts passing directly; typecheck, lint, and i18n passed. Full integration execution remains affected by the repository's shared-database schema teardown race, and the expanded Playwright spec was environment-blocked by a stale localhost process; these are verification-environment limitations rather than accepted feature behavior.
- **Operational notes:** Apply both migrations `0028` and `0029`. Legacy courses may retain null credits but cannot produce records until configured. Migration `0029`'s rollback is intentionally irreversible after valid feature use. Student reads are self-scoped, instructor reads require active section enrollment, and admin provenance fields remain restricted.

### TRACK-061: Institutional Reporting & Secure Delivery

- **Status:** ✅ Complete, reviewed, and archived in `conductor/archive/institutional_reporting_secure_delivery_20260809/` · **Audit IDs:** None (implemented from the TRACK-019/PRD PDF deferral) · **Deps:** TRACK-019, TRACK-057, TRACK-060
- **Estimated Effort:** Delivered; recurring scheduling was deliberately removed from this slice.
- **Context anchors:** `conductor/archive/institutional_reporting_secure_delivery_20260809/`; `docs/PRD.md` — Analytics & Reporting; implementation in `src/server/reporting*.server.ts`, `src/db/schema/report-jobs.ts`, and `src/components/reporting/`
- **Problem:** TRACK-019 supplied analytics and CSV/Excel exports but not a secure PDF boundary. Institutional reports needed authoritative academic context, immutable transcript data, durable status, private artifact storage, expiry, and role-scoped access without becoming an arbitrary report builder.
- **Decisions:** Support exactly three allowlisted reports: Institutional Academic Summary, Official Transcript, and Analytics Summary. Generate reports manually on demand only; recurring schedules, cron infrastructure, saved presets, and email attachments remain deferred. Use server-only PDFKit with bundled Noto Sans fonts, private opaque `reports/` R2 keys, five-minute authorized downloads, durable report jobs, and 30-day expiry. Course sections carry optional explicit cohort metadata; cohort is never inferred from names or codes.
- **Scope delivered:** Admin/superadmin institutional reports; instructor analytics restricted to active enrolled sections; student self-transcripts and authorized admin transcript generation; term/course/section/cohort filters; bilingual PDFs and accessible role routes; durable pending/processing/completed/failed/expired jobs; retries, stale-processing failure recovery, audit-safe status/history, private downloads, and idempotent operational cleanup.
- **Out of Scope:** Recurring or scheduled delivery, arbitrary report/SQL builders, saved presets, email attachments, external institutional delivery integrations, mutable or unreleased transcript data, and permanent/public artifact URLs.
- **Execution result:** Migrations `0030`–`0032` add report jobs, explicit section cohort metadata, and retry-safe expiry constraints. PDF rendering and R2 operations remain outside database transactions; guarded transitions prevent duplicate workers; uploaded artifacts are cleaned on failed completion; stale processing jobs become safely retryable failures; production builds copy and smoke-test the embedded fonts. A required R2 lifecycle rule deletes orphaned `reports/` objects after 40 days.
- **Verification:** Project coverage remained above 80%; fresh-database integration tests, the production build/font smoke, and 36 reporting Playwright tests across Chromium, Firefox, and mobile Chrome passed. The principal review findings were remediated before archival.

### TRACK-062: Risk History & Student Success

- **Status:** 🚧 In progress · **Audit IDs:** None (explicit PRD `[v2]` deferral) · **Deps:** TRACK-023 and TRACK-050 (complete risk and intervention foundations); coordinated with TRACK-057, TRACK-058, and TRACK-061
...
- **Execution result:** Implemented immutable, idempotent lifecycle and daily risk observations using the existing scoring authority; bounded retry-safe five-academic-year anonymization; reassignment-safe instructor history and outcome views; admin-only aggregates with ten-student suppression; and a constructive self-only student support projection. Capture and scheduled processing remain advisory and create neither notifications nor automatic academic actions. Responsive bilingual instructor, admin, and student surfaces include loading, empty, error, filtering, pagination, and accessibility states. Final release verification and archival remain in progress.
...
| **AE** | TRACK-061 → TRACK-057, TRACK-060 | Complete. Reporting uses TRACK-057 academic context and explicit cohort metadata plus TRACK-060 immutable released records; recurring scheduling remains a separate future decision. |
